...
function applyLeadFilters(query: ReturnType<typeof supabase.from>, filters: AnalyticsFilters) {
  let q = query;
...
async function getGenericPartData(
  ...
  let filteredRows =
    table === "lead"
      ...
      : rows
          .filter((r) => leadsMap.has(r[joinCol]))
          .map((r) => ({ ...r, lead: leadsMap.get(r[joinCol]) }));
