...
async function main() {
  ...
  // 2. Llamadas
  const { data: llamadas, error: errLlamadas } = await supabase
    .from("llamadas")
    .select("*")
    .eq("tenant_id", tenantId);
  ...
  // 3. Appointments
  const { data: appointments, error: errApps } = await supabase
    .from("appointments")
    .select("*")
    .eq("tenant_id", tenantId);
  ...
  // 4. Agendamientos
  const { data: agendamientos, error: errAgendamientos } = await supabase
    .from("agendamientos")
    .select("*")
    .eq("tenant_id", tenantId);
  ...
  const { data: cualificaciones, error: errCual } = await supabase
