...
export class FactExtractionService {
  ...
  static async extractFromDialogue(
    ...
    console.log(
      `[FACT EXTRACTOR] 🧠 Extracting for lead ${leadId}: [${normalizedKeys.join(", ")}]`
    );
    ...
    try {
      ...
      let finalizationRules =
        "El usuario se despidió, la cita quedó confirmada, o se descartó explícitamente.";
...
{"user_name": "Carlos", "RESUMEN_EJECUTIVO": "Interesado en MBA", "qualified": "SI", "segmentacion": "${validSegments[0] || "REVISADO"}", "estado_conversacion": "FINALIZADA", "ESTADO": "Interesado", "REGLA_APLICADA": "Sin requisitos", "QA_HANDLED": "SI", "QA_TOPIC": "Precios", "CURSE_NAME": null}`;
      ...
      const extractorModel = trimmedKey.startsWith("sk-or-v1")
        ? "openai/gpt-4o-mini"
        : "gpt-4o-mini";
      ...
      Object.entries(extractedData).forEach(([key, val]) => {
        if (
          ...
          const trackedMatch = varsToTrack.find((v) => {
            const normalizedV = FactExtractionService.normalizeKey(v)
              .toLowerCase()
              .replace(/\s+/g, "");
            return (
              normalizedV === key.toLowerCase().replace(/\s+/g, "") ||
              v.toLowerCase().replace(/\s+/g, "") === key.toLowerCase().replace(/\s+/g, "")
            );
          });
      ...
      if (Object.keys(result).length > 0) {
        ...
        if (tenantId) {
          ...
          if (
            convStatus === "FINALIZADA" ||
            convStatus === "FINALIZADO" ||
            convStatus === "CLOSED"
          ) {
            const { enqueueQualificationAnalysis } =
              await import("@/lib/core/queue/lead-sequence-queue");
  ...
  private static async saveToLeadMetadata(leadId: string, newData: Record<string, string>) {
    ...
    const { data: leadFound, error: fetchError } = (await supabase
      .from("lead")
    ...
    if (updateError) {
      ...
      const exp =
        ...
        meta.YEARS_EXPERIENCE ||
        meta.YEARS_EXPERIENCIE ||
        meta["YEARS_ EXPERIENCIE"];
      ...
      if (studies && exp) {
        ...
        if (result.status === "cualificado") {
          ...
          console.log(
            `[FACT_EXTRACTOR] ℹ️ Lead ${leadId} evaluated but NOT qualified yet: ${result.reason}`
          );
          const { data: lead } = (await supabase
            .from("lead")
            .select("tenant_id")
