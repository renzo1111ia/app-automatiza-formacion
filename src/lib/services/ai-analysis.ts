...
async function resolveApiKey(tenantId?: string): Promise<string> {
  // 1. Try Supabase first (DI: usa getAdminSupabaseClient centralizado, sin createClient inline).
  if (tenantId) {
    try {
      const supabase = await getAdminSupabaseClient();
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const { data } = await (supabase.from("ai_agent_variants") as any)
        .select("api_key")
      ...
      if (
        ...
        String((data as Record<string, unknown>).api_key).startsWith("sk-")
      ) {
  ...
  throw new Error(
    "No se encontró una API key de OpenAI. Configúrala en el agente activo dentro del dashboard."
  );
}
