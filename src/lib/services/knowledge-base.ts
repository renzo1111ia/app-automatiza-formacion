...
export class KnowledgeBaseService {
  ...
  static async search(
    ...
    try {
      ...
      )("match_knowledge_base", rpcArgs);

      if (error) {
        // Check if it's a schema cache issue
        if (
          error.message?.includes("function") &&
          error.message?.includes("match_knowledge_base")
        ) {
          console.warn(
            "[KNOWLEDGE BASE] match_knowledge_base function not found in cache. This is a known Supabase issue. Proceeding with empty results."
          );
          return [];
        }
   ...
   */
  static async addEmbeddingsBatch(
    tenantId: string,
    items: Array<{
      content: string;
      embedding: number[];
