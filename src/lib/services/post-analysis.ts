...
export class PostAnalysisService {
  ...
  static async processInteraction(params: {
    ...
    const { leadId, tenantId, transcript, channel, externalId, durationMs, disconnectionReason } =
      params;
    ...
    try {
      ...
      if (channel === "CALL") {
        ...
        if (disconnectionReason && INVALID_REASONS.includes(disconnectionReason))
          isUnproductive = true;
      }

      // 2. Extract Data with AI (if productive)
      const analysis =
        !isUnproductive && transcript.length > 50
          ...
          : {
              ...
              reasons:
                disconnectionReason ||
                (isUnproductive ? "Llamada no productiva" : "Conversación insuficiente"),
              student_interest_level: 0,
              lead_score: 0,
              extracted_data: {
                motivo_anulacion:
                  disconnectionReason === "invalid_destination" ? "teléfono falso" : "Ilocalizable",
              },
            };
      ...
      await supabase.from("lead_cualificacion").upsert({
        tenant_id: tenantId,
        id_lead: leadId,
        cualificacion: analysis.qualified,
        motivo_anulacion:
          analysis.extracted_data.MOTIVO_DESCARTE || analysis.extracted_data.motivo_anulacion,
        anios_experiencia:
          analysis.extracted_data["YEARS_EXPERIENCE"] ||
          analysis.extracted_data["YEARS_ EXPERIENCIE"] ||
          analysis.extracted_data["YEARS_EXPERIENCIE"] ||
          analysis.extracted_data.años_experiencia,
        nivel_estudios:
          analysis.extracted_data.USER_ESTUDIES || analysis.extracted_data.nivel_estudios,
        calificacion_score: analysis.lead_score,
        analisis_profundo: {
          reasons: analysis.reasons,
          interest_level: analysis.student_interest_level,
          titulacion:
            analysis.extracted_data.USER_PROFESION || analysis.extracted_data.titulacion_lead,
      ...
      if (transcript.length > 50 && !isUnproductive) {
        try {
          const { data: agentVariant } = (await supabase
            ...
            data: { api_key: string; tracked_variables: string[] } | null;
          };

          const apiKey = (agentVariant as { api_key?: string } | null)?.api_key;
          const trackedVars =
            (agentVariant as { tracked_variables?: string[] } | null)?.tracked_variables || [];

          if (apiKey && trackedVars.length > 0) {
            console.log(
              `[POST-ANALYSIS] Running fact extraction for ${trackedVars.length} tracked vars`
            );
            ...
            console.log(
              `[POST-ANALYSIS] Fact extraction captured: ${Object.keys(factData).join(", ")}`
            );
          }
        } catch (factErr) {
      ...
      await supabase.from("lead").update(mainUpdate).eq("id", leadId);

      // 5. Handle Scheduling / Retries
      if (
        analysis.qualified === "si" &&
        (analysis.scheduled_call_confirmed || analysis.extracted_data.date_time_preferred)
      ) {
        ...
        const lastAttemptNum =
          (lastAttempts as Array<{ numero_intento: number }> | null)?.[0]?.numero_intento ?? 0;

        if (lastAttemptNum < 4) {
          const nextRetryDate = SchedulerService.calculateNextRetry(
            new Date(),
            lastAttemptNum,
            lead.pais || "España"
          );
          ...
          console.log(
            `[POST-ANALYSIS] Scheduled retry #${lastAttemptNum + 1} for ${nextRetryDate.toISOString()}`
          );
        } else {
