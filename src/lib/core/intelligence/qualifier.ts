...
const _EXP_RANGES = ["0-5 años", "5-10 años", "10-20 años", "+20 años", "N/A"];
...
function categorizeStudies(text: string): keyof typeof NIVELES_ESTUDIOS {
  ...
  if (/licenciatura|grado|ingenier[íi]a|universidad|universitario|bachiller en/.test(s))
...
export function evaluateLeadQualification(input: LeadQualificationInput): QualificationResult {
  ...
  const expYears =
    typeof input.years_experience === "string"
      ? parseInt(input.years_experience.replace(/\D/g, ""))
