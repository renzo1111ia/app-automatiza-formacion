...
export class SweepQueue {
  ...
  public async enqueue(
    ...
    try {
      ...
      console.log(
        `[SWEEP QUEUE] Lead ${leadId} queued for ${actionType} at ${scheduledAt.toISOString()}`
      );
    } catch (error: unknown) {
      const msg = error instanceof Error ? error.message : String(error);
      console.warn(`[SWEEP_QUEUE_BYPASS] Could not enqueue lead: ${msg}`);
    }
  }
  ...
  public async sweep() {
    try {
      ...
      const msg = error instanceof Error ? error.message : String(error);
      console.warn(`[SWEEP_QUEUE_OFFLINE] Cannot sweep: ${msg}`);
    }
  }
}
