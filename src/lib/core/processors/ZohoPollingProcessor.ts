import { zohoClient, type ZohoLead } from "../../integrations/zoho";
...
export class ZohoPollingProcessor {
  async run() {
    ...
    for (const tenant of tenants as Tenant[]) {
      try {
        ...
        const criteria =
          "(Lead_Status:equals:Nuevo) and (Lead_Source:equals:Meta) and (Tag:not_contains:VirginIA)";
        const externalLeads = await zohoClient.searchLeads(criteria);

        if (externalLeads.length === 0) continue;

        console.log(
          `[ZOHO_POLLER] Found ${externalLeads.length} leads. Filtering by Line of Business...`
        );

        for (const extLead of externalLeads as ZohoLead[]) {
          try {
            // Extra Filter: L_nea_de_Negocio (n8n logic)
            const ldn = extLead.L_nea_de_Negocio as string | undefined;
            if (ldn === "GenD" || ldn === "EAP") {
            ...
            const rawPhone = (extLead.Phone as string | undefined) ?? "";
              ...
              .upsert(
                {
                  ...
                  nombre: (extLead.First_Name as string | undefined) ?? "Lead",
                  apellido: (extLead.Last_Name as string | undefined) ?? "Zoho",
                  telefono: normalizedPhone,
                  email: (extLead.Email as string | undefined) ?? "",
                  pais: (extLead.Country as string | undefined) ?? "España",
                  origen: (extLead.Lead_Source as string | undefined) ?? "Zoho",
                  tipo_lead: "nuevo",
                  metadata: {
            ...
            const masterName = extLead.Productos as string | undefined; // n8n field for program
            if (masterName) {
  ...
  private async matchAndLinkProgram(
    ...
    if (program) {
      ...
      console.log(
        `[ZOHO_POLLER] Linked lead ${leadId} to program ${(program as { id: string }).id} (${masterName})`
      );
    } else {
