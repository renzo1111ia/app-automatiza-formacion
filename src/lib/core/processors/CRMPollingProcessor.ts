...
import { resolveLeadCountry } from "../../integrations/sheets/phone-country";
  ...
  async run() {
    ...
    for (const tenant of tenants as Tenant[]) {
      try {
        ...
        const crmConfig = config.crm as
          | { enabled?: boolean; provider?: string; search_criteria?: string }
          | undefined;
              ...
              .upsert(
                {
                  ...
                  email: fields.email || "",
                  // País (regla AF): explícito → teléfono → España.
                  pais: resolveLeadCountry(
                    fields.pais as string | undefined,
                    fields.telefono as string | undefined
                  ),
