...
export async function fetchCalls({
  ...
  try {
    ...
    // Sprint 0 tarea 1-20: filtro tenant_id obligatorio. Sin tenant activo,
    // no devolvemos datos (antes fugaba todos los tenants — F-04-001).
    ...
    if (leadIds.length > 0) {
      ...
      if (appts) {
        (
          appts as { lead_id: string; scheduled_at: string; status: string; created_at: string }[]
        ).forEach((apt) => {
    ...
    const rows: HistorialRow[] = ((data as unknown as JoinedLead[]) ?? []).map((lead) => {
      const sortedLlamadas = (lead.llamadas ?? []).sort(
        (a, b) => new Date(b.fecha_inicio || 0).getTime() - new Date(a.fecha_inicio || 0).getTime()
      );
      ...
      const sortedAppointments = leadAppointments.sort(
        (a, b) => new Date(b.created_at || 0).getTime() - new Date(a.created_at || 0).getTime()
      );
      const latestAppt = sortedAppointments[0] || {};

      const latestWA =
        (lead.conversaciones_whatsapp ?? []).sort(
          (a, b) =>
            new Date(b.fecha_ultimo_mensaje || 0).getTime() -
            new Date(a.fecha_ultimo_mensaje || 0).getTime()
      ...
      if (lead.fecha_ingreso_crm && firstCall.fecha_inicio) {
        const diff =
          new Date(firstCall.fecha_inicio).getTime() - new Date(lead.fecha_ingreso_crm).getTime();
...
export async function getCallsByPhone(phone: string): Promise<HistorialRow[]> {
  try {
    const supabase = await getSupabaseServerClient();
    // Sprint 0 tarea 1-20: filtro tenant_id obligatorio.
    const tenantId = await getActiveTenantId();
    if (!tenantId) return [];
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .select(
        `
                *,
                lead:id_lead!inner ( id, nombre, apellido, telefono, tenant_id )
            `
      )
...
export async function fetchWhatsappByPhone(phone: string) {
  try {
    ...
    const { data, error } = await supabase
      ...
      .select(`*, lead:id_lead!inner ( id, nombre, apellido, telefono, tenant_id )`)
      .eq("lead.tenant_id", tenantId)
      .eq("lead.telefono", phone)
