...
export interface KpiGenerales {
  ...
  total_whatsapp_conversaciones: number; // Sprint 2B: count of wpData rows in window
}
...
export async function getKpiGenerales(
  ...
  try {
    ...
    const reachedSet = new Set(
      [...llamadasData.map((l: any) => l.lead?.id), ...wpData.map((w: any) => w.id_lead)].filter(
        Boolean
      )
    );

    const total_cualificados = cualData.filter(
      (c: any) => c.cualificacion && c.cualificacion !== "NO"
    ).length;
    const total_leads = leadsData.length;

    const allReasons: any[] = [];
    cualData.forEach((c: any) => {
      if (c.motivo_anulacion) allReasons.push({ m: c.motivo_anulacion });
    });
    llamadasData.forEach((l: any) => {
      if (l.estado_llamada !== "CONTACTED" && l.razon_termino)
        allReasons.push({ m: l.razon_termino });
    });

    const agendaMap: Record<string, number> = {};
    agendaData.forEach((a: any) => {
      if (a.fecha_agendada_cliente) {
        const d = a.fecha_agendada_cliente.slice(0, 10);
        agendaMap[d] = (agendaMap[d] || 0) + 1;
      }
    });
    ...
    return {
      ...
      duracion_media_segundos: llamadasData.length
        ? Math.round(totalSecs / llamadasData.length)
        : 0,
      tasa_contacto: llamadasData.length
        ? Math.round((contactados.length / llamadasData.length) * 100)
        : 0,
      ...
      tasa_ilocalizables: total_leads
        ? Math.round(
            (leadsData.filter((l) => l.tipo_lead === "ilocalizable").length / total_leads) * 100
          )
        : 0,
      ...
      agendados_por_fecha: Object.entries(agendaMap)
        .map(([label, value]) => ({ label, value }))
      ...
      total_whatsapp_conversaciones: wpData.length,
    };
  } catch {
    return empty;
  }
...
export async function getKpiMinutos(
  ...
  try {
    ...
    rows.forEach((l) => {
      if (l.fecha_inicio) {
        const d = l.fecha_inicio.slice(0, 10);
        byDay[d] = (byDay[d] || 0) + (l.duracion_segundos || 0);
      }
    });
    return {
      ...empty,
      ...
      minutos_por_dia: Object.entries(byDay)
        .map(([label, value]) => ({ label, value }))
        .sort((a, b) => a.label.localeCompare(b.label)),
    };
  } catch {
...
export async function getKpiWhatsapp(
  ...
  try {
    ...
    const filteredWpRows = wpRows
      .filter((r: any) => leadsMap.has(r.id_lead))
      .map((r: any) => ({
        ...r,
    ...
    const uniqueCualificados = new Set(
      cualData
        .filter((q: any) => q.cualificacion && q.cualificacion !== "NO")
        .map((q: any) => q.id_lead)
    ).size;

    const total_ilocalizables = filteredWpRows.filter(
      (r: any) => r.lead?.tipo_lead === "ilocalizable"
    ).length;
    ...
    return {
      ...
      tasa_agendamiento: total_leads_unicos
        ? Math.round((uniqueAgendados / total_leads_unicos) * 100)
        : 0,
      tasa_conversion: total_leads_unicos
        ? Math.round((uniqueCualificados / total_leads_unicos) * 100)
        : 0,
      tasa_ilocalizables: total_leads_unicos
        ? Math.round((total_ilocalizables / total_leads_unicos) * 100)
        : 0,
      por_estado_conversacion: groupBy(filteredWpRows, "estado"),
      por_tipo_lead: groupBy(
        filteredWpRows.map((r) => r.lead),
        "tipo_lead"
      ),
      por_origen: groupBy(
        filteredWpRows.map((r) => r.lead),
      ...
      conversaciones_por_dia: Object.entries(byDay)
        .map(([label, value]) => ({ label, value }))
        .sort((a, b) => a.label.localeCompare(b.label)),
    };
  } catch (err) {
...
export async function getKpiCampanas(
  ...
  const empty: KpiCampanas = {
    campanas: [],
    leads_por_campana: [],
    contactados_por_campana: [],
    cualif_por_campana: [],
    agendados_por_campana: [],
    minutos_por_campana: [],
    total_leads: 0,
    total_llamadas: 0,
    total_contactados: 0,
  ...
  try {
    ...
    const reachedSet = new Set(
      [
        ...(llRes.data || [])
          .filter((l: any) => leadsMap.has(l.id_lead))
          .map((l: any) => l.id_lead),
    ...
    return {
      campanas,
      leads_por_campana: campanas.map((c: any) => ({ label: c.nombre, value: c.total_leads })),
      contactados_por_campana: campanas.map((c: any) => ({
        label: c.nombre,
        value: c.contactados,
      })),
...
async function getGenericPartData(
  supabase: any,
  part: any,
  from: string,
  to: string,
  ...
  let filteredRows =
    table === "lead"
      ...
      : rows
          .filter((r) => leadsMap.has(r.id_lead))
          .map((r) => ({ ...r, lead: leadsMap.get(r.id_lead) }));
  ...
  if (part.calcType === "avg")
    return filteredRows.length
      ? filteredRows.reduce((s, r) => s + (Number(r[col]) || 0), 0) / filteredRows.length
      : 0;

  return 0;
}

export async function getSingleDynamicKpi(
  kpi: KpiConfig,
  from: string,
  to: string,
  filters: AnalyticsFilters
  ...
  try {
    ...
    if (kpi.isAdvanced && kpi.parts) {
      ...
      const results = await Promise.all(
        partKeys.map((k) => getGenericPartData(supabase, kpi.parts![k], from, to, filters))
      );
      const values: any = {};
      ...
      num = await getGenericPartData(
        supabase,
        {
          targetCol: kpi.targetCol || "lead.id",
          calcType: kpi.calcType || "count",
          condCol: kpi.condCol,
          condOp: kpi.condOp,
          condVal: kpi.condVal,
        },
      ...
      if (kpi.isPercentage) {
        const den = await getGenericPartData(
          supabase,
          {
            targetCol: kpi.denomTargetCol || "lead.id",
            calcType: kpi.denomCalcType || "count",
            condCol: kpi.denomCondCol,
            condOp: kpi.denomCondOp,
            condVal: kpi.denomCondVal,
          },
          from,
          to,
          filters
  ...
  const results = await Promise.all(
    charts.map(async (c) => {
      try {
        ...
        const parts: Record<string, any> = c.isAdvanced
          ? c.parts || {}
          : { a: { targetCol: c.dataKey || "lead.id", calcType: "count" } };
        const partKeys = Object.keys(parts);
        const partData = await Promise.all(
          partKeys.map((k) => getGenericPartData(supabase, parts[k], from, to, filters, true))
        );
    ...
    dynamicCharts.map(async (c) => {
      try {
        ...
        if (c.type === "heatmap") {
          ...
          console.log(
            `[getDynamicChartSeries] Success for Heatmap "${c.title}": ${data.length} data points.`
          );
          return { id: c.id, data };
        }
        ...
        if (series.length > 0) {
          console.log(
            `[getDynamicChartSeries] Success for "${c.title}": ${rows.length} rows, base=${baseTable}, x=${xTable}.${xCol}`
          );
        }
...
// ─── SPRINT 2B: OVERVIEW (cross-canal hero) ─────────────────────────────────

/**
 * Thin wrapper sobre getKpiGenerales() para la sección Overview del dashboard.
 * Reusa las 5 queries paralelas existentes — cero queries nuevas.
 * Validación Zod en input + output (fail-fast si shape inesperado).
 *
 * Mapping puro extraído a `mapKpiGeneralesToOverview` en
 * `src/lib/mappers/kpi-overview.ts` para testeo sin Supabase.
 */
export async function getKpiOverview(
  ...
  const { mapKpiGeneralesToOverview } = await import("@/lib/mappers/kpi-overview");
  const input = KpiOverviewInputSchema.parse({ from, to, filters });
  const kpi = await getKpiGenerales(input.from, input.to, input.filters as AnalyticsFilters);
  return mapKpiGeneralesToOverview(kpi);
}
