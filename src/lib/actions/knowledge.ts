 ...
 * Delegates to the shared processKnowledgeUpload service (no "use server" boundary issues).
 */
export async function uploadKnowledgeDocument(formData: FormData) {
  const { processKnowledgeUpload } = await import("@/lib/services/knowledge-upload-service");
  return processKnowledgeUpload(formData);
}
