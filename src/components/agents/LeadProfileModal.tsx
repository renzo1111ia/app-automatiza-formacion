    ...
    return (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 md:p-6">
            ...
            <motion.div 
                ...
                className="relative w-full max-w-4xl max-h-[90vh] bg-[#0b0e14] border border-white/10 rounded-[40px] shadow-[0_0_100px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col"
            >
                {/* Header */}
                <div className="p-8 md:p-10 border-b border-white/5 flex items-center justify-between bg-black/20">
                    <div className="flex items-center gap-6">
                        ...
                        <div>
                            <h2 className="text-2xl font-black uppercase tracking-tight text-white">Perfil del Lead</h2>
                            <p className="text-[11px] font-bold text-white/20 uppercase tracking-[0.3em] mt-1">Gestión y Datos Capturados</p>
                        </div>
                    </div>
                    <button 
                        title="Cerrar"
                        onClick={onClose} 
                        className="h-12 w-12 flex items-center justify-center rounded-2xl hover:bg-white/5 transition-all"
                    >
                                    ...
                                    Object.entries(metadata).map(([key, value]) => (
                                        <div key={key} className="relative group">
                                            <div className="flex items-center justify-between mb-2 px-1">
                                                <label className="text-[10px] font-black uppercase tracking-widest text-white/30">{key.replace(/_/g, ' ')}</label>
                                                <button 
                                            ...
                                            <input 
                                                ...
                                                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm font-bold text-white focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all"
                                            />
                                        </div>
                                    ))
                                )}
                ...
                {/* Footer */}
                <div className="p-8 md:p-10 border-t border-white/5 bg-black/40 flex items-center justify-end gap-4">
                    <button 
                        onClick={onClose}
                        className="h-14 px-8 rounded-2xl bg-white/5 border border-white/5 text-[11px] font-black uppercase tracking-widest text-white/40 hover:bg-white/10 transition-all"
                    >
                        Cancelar
                    </button>
                    <button 
...
function InputField({ label, value, onChange, icon: Icon }: { label: string, value: string, onChange: (val: string) => void, icon?: LucideIcon | React.ElementType }) {
    return (
        <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-widest text-white/20 px-1">{label}</label>
            <div className="relative group">
                {Icon && <Icon className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-white/10 group-focus-within:text-primary transition-colors" />}
                <input 
                    value={value}
                    onChange={(e) => onChange(e.target.value)}
                    title={label}
                    className={cn(
                        "w-full bg-white/5 border border-white/10 rounded-2xl py-4 text-sm font-bold text-white focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all",
    ...
    return (
        <div className="p-4 rounded-2xl bg-white/[0.02] border border-white/5 flex flex-col gap-1">
            ...
            <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] font-bold text-white/60 truncate">{value}</span>
            </div>
        </div>
    );
}
