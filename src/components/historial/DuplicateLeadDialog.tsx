...
function CallCard({ call }: { call: HistorialRow }) {
  ...
  return (
    <div className="border-border bg-muted/20 group hover:border-primary/30 relative overflow-hidden rounded-xl border p-4 transition-colors">
      <div className="absolute top-0 left-0 h-full w-1 bg-emerald-500/20 transition-colors group-hover:bg-emerald-500/50" />

      {/* Header */}
      <div className="mb-3 flex flex-wrap items-center gap-2 pl-2">
        <span className="flex items-center gap-1.5 rounded-full border border-emerald-500/20 bg-emerald-500/10 px-2.5 py-0.5 text-xs font-medium text-emerald-600 dark:text-emerald-400">
        ...
        {call.duracion_segundos != null && (
          <span className="text-muted-foreground bg-muted flex items-center gap-1 rounded px-2 py-0.5 text-xs">
            <Clock className="h-3 w-3" />
            {formatDuration(call.duracion_segundos)}
          </span>
        )}
        <span className="text-muted-foreground/60 ml-auto flex items-center gap-1 text-xs">
      ...
      {/* Main fields */}
      <div className="grid grid-cols-2 gap-x-4 gap-y-2 pl-2 text-xs">
        {call.razon_termino && (
          <div>
        ...
        {call.cualificacion && (
          <div>
            ...
            <span
              className={
                call.cualificacion === "SI" || call.cualificacion === "CUALIFICADO"
        ...
        {call.fecha_agendada_cliente && (
          <div>
            ...
            <span className="font-bold text-teal-600 dark:text-teal-400">
              {formatDate(call.fecha_agendada_cliente)}
            </span>
          </div>
        )}
        {call.tipo_agente && (
      ...
      {(call.resumen || call.url_grabacion) && (
        <>
          <button
            ...
            className="text-muted-foreground/60 hover:text-primary mt-3 flex items-center gap-1 pl-2 text-xs transition"
          >
            {expanded ? <ChevronUp className="h-3 w-3" /> : <ChevronDown className="h-3 w-3" />}
            {expanded ? "Ocultar detalle" : "Ver resumen / audio"}
          </button>
          {expanded && (
            <div className="border-border mt-3 space-y-3 border-t pt-3 pl-2">
              {call.resumen && (
                <div>
                  <span className="text-muted-foreground/40 mb-1 block text-xs">Resumen</span>
                  <p className="text-card-foreground/80 text-xs leading-relaxed whitespace-pre-wrap">
                    {call.resumen}
                  </p>
                </div>
              )}
...
function IntentoCard({ intento }: { intento: IntentoLlamada }) {
  ...
  return (
    <div className="border-primary/20 bg-primary/5 group hover:border-primary/40 relative overflow-hidden rounded-xl border p-4 transition-colors">
      <div className="bg-primary/30 group-hover:bg-primary/60 absolute top-0 left-0 h-full w-1 transition-colors" />

      <div className="mb-3 flex flex-wrap items-center gap-2 pl-2">
        <span className="text-primary bg-primary/10 border-primary/20 flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-xs font-medium">
          <RefreshCw className="h-3 w-3" /> Intento
        </span>
        {intento.tipo_intento && (
          <span className="bg-muted text-muted-foreground border-border rounded-full border px-2 py-0.5 text-xs">
            {intento.tipo_intento}
          </span>
        )}
        <span className="text-muted-foreground/40 ml-auto flex items-center gap-1 text-xs">
          <Calendar className="h-3 w-3" />
...
export function DuplicateLeadDialog({ phone, onClose }: Props) {
  ...
  return (
    <div className="bg-background/80 fixed inset-0 z-50 flex items-center justify-end backdrop-blur-sm sm:p-4">
      <div className="bg-card border-border animate-in slide-in-from-right relative flex h-full w-full max-w-3xl flex-col overflow-hidden border shadow-2xl sm:min-h-0 sm:rounded-2xl">
        {/* Header */}
        <div className="border-border bg-muted/30 flex items-center justify-between border-b px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-card-foreground text-lg font-semibold">Historial del número</h2>
              <p className="text-primary mt-0.5 font-mono text-xs">{phone}</p>
            </div>
          </div>
          <button
            onClick={onClose}
            title="Cerrar"
            aria-label="Cerrar"
            className="text-muted-foreground hover:bg-muted hover:text-card-foreground rounded-full p-2 transition"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        {!loading && timeline.length > 0 && (
          <div className="border-border bg-muted/10 flex flex-wrap items-center gap-3 border-b px-6 py-3 text-xs">
            <span className="text-muted-foreground/60">
              {timeline.length} interacciones encontradas
            </span>
            {calls.length > 0 && (
              <span className="flex items-center gap-1.5 rounded-full border border-emerald-500/20 bg-emerald-500/10 px-2.5 py-0.5 text-emerald-600 dark:text-emerald-400">
                <PhoneCall className="h-3 w-3" /> {calls.length} llamada{calls.length !== 1 && "s"}
              </span>
            )}
            {intentos.length > 0 && (
              <span className="text-primary bg-primary/10 border-primary/20 flex items-center gap-1.5 rounded-full border px-2.5 py-0.5">
                <RefreshCw className="h-3 w-3" /> {intentos.length} intento
                {intentos.length !== 1 && "s"}
              </span>
            )}
          </div>
        ...
        <div className="flex-1 space-y-4 overflow-y-auto p-6">
          {loading ? (
            ...
            <div className="text-muted-foreground/60 py-20 text-center">
