  ...
  const updatedKpis = config.kpis.map((kpi: any) => {
    ...
    switch (kpi.id) {
      case "def-2": // Leads localizados
        console.log(
          `Migrating ${kpi.id} ("${kpi.label}") to dynamic: count llamadas.id_lead where estado_llamada = CONTACTED`
        );
        return {
          ...base,
      ...
      case "def-4": // Tasa de contacto (%)
        console.log(
          `Migrating ${kpi.id} ("${kpi.label}") to dynamic: percentage of calls contacted`
        );
        return {
          ...base,
          targetCol: "llamadas.id_lead",
      ...
      case "def-7": // Citas Agendadas
        console.log(
          `Migrating ${kpi.id} ("${kpi.label}") to dynamic: count appointments.id where status = CONFIRMED`
        );
        return {
          ...base,
      ...
      case "def-12": // Tasa de agenda (%)
        console.log(
          `Migrating ${kpi.id} ("${kpi.label}") to dynamic: confirmed appointments / total leads`
        );
        return {
          ...base,
          targetCol: "appointments.id",
      ...
      case "def-8": // Leads Cualificados
        console.log(
          `Migrating ${kpi.id} ("${kpi.label}") to dynamic: count lead_cualificacion.id where cualificacion != NO`
        );
        return {
          ...base,
      ...
      case "def-13": // Tasa de conversión (%)
        console.log(
          `Migrating ${kpi.id} ("${kpi.label}") to dynamic: qualified leads / total leads`
        );
        return {
          ...base,
          targetCol: "lead_cualificacion.id",
      ...
      case "def-10": // Descartados
        console.log(
          `Migrating ${kpi.id} ("${kpi.label}") to dynamic: count lead.id where tipo_lead = descartado`
        );
        return {
          ...base,
          targetCol: "lead.id",
      ...
      case "def-14": // % Ilocalizables (%)
        console.log(
          `Migrating ${kpi.id} ("${kpi.label}") to dynamic: count lead.id where tipo_lead = ilocalizable / total leads`
        );
        return {
          ...base,
          targetCol: "lead.id",
