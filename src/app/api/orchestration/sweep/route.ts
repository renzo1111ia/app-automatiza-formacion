 ...
 *
 * Sprint 0 tarea 1-08: requiere header `x-cron-secret: <CRON_SECRET>` o
 * `Authorization: Bearer <CRON_SECRET>` para invocar. CRON_SECRET en env.
 */
export async function GET(req: Request) {
  ...
  try {
    ...
    for (const actionRaw of actions || []) {
      ...
      try {
        console.log(
          `[SWEEP] Processing action ${action.id} (${action.action_type}) for lead ${action.lead_id}`
        );
