 ...
 *
 * Sprint 0 tarea 1-08: requiere header `x-cron-secret` o
 * `Authorization: Bearer <CRON_SECRET>`. CRON_SECRET en env.
 */
    ...
    for (const apt of appts) {
      try {
        ...
        if (config?.mode === "ai") {
          ...
          const apiKey =
            (tenant?.config as TenantConfig | undefined)?.openai?.api_key ||
            ...
            messages: [
              {
                role: "system",
                content:
                  "Eres un asistente de Esden Business School. Tu tarea es generar un recordatorio de cita corto, amable y profesional para WhatsApp. No uses negritas excesivas ni emojis exagerados. Máximo 250 caracteres.",
              },
              {
                role: "user",
                content: `Genera un recordatorio para:
                                Lead: ${lead.nombre}
          ...
          const template =
            config?.template ||
            "Hola {nombre}, te recordamos tu cita de hoy a las {hora} (España). ¡Te esperamos!";
          message = template
