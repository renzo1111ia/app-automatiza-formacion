 ...
 *
 * Sprint 0 tarea 1-13: firma HMAC obligatoria — este endpoint puede agendar,
 * cancelar y reprogramar citas (el más peligroso sin auth).
 */
export async function POST(req: Request) {
  try {
    ...
    let payload: {
      ...
      call?: { metadata?: Record<string, string> };
    };
    try {
    ...
    if (!leadId || !tenantId) {
      console.warn(
        "[RETELL TOOLS] Missing lead_id or tenant_id in call metadata. Using fallback search."
      );
...
async function handleBookAppointment(
  ...
  if (allAdvisors && allAdvisors.length > 0) {
    ...
    selectedAdvisor =
      allAdvisors.find(
        (a) =>
          a.specialties?.includes(programId || "") || a.specialties?.includes(programName || "")
      ) || null;
