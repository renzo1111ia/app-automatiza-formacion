...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_APP_URL || "http://localhost:8500"}/dashboard/settings?google=success`
    );
  } catch (error) {
    console.error("[GOOGLE CALLBACK] Error:", error);
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_APP_URL || "http://localhost:8500"}/dashboard/settings?google=error`
    );
  }
}
