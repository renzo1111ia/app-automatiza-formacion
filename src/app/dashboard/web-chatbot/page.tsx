            ...
            <div className="flex items-center justify-between px-8 py-6 bg-card/20 border-b border-border">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-muted-foreground font-bold uppercase tracking-widest leading-none mt-1">Conecta tus agentes de IA a burbujas de chat web.</p>
                    </div>
                </div>
                        ...
                        {widgets.map(w => (
                            <div
                                ...
                                className={cn(
                                    ...
                                    selectedWidget?.id === w.id ? "bg-primary/10 border-primary/20" : "bg-card/40 border-border hover:bg-card/60"
                                )}
                            >
                                ...
                                <p className="text-[10px] text-muted-foreground truncate">{agents.find(a => a.id === w.agent_id)?.name || "Sin agente vinculado"}</p>
                            </div>
                        ))}
                    </div>
                </div>
                        ...
                        <div className="max-w-4xl mx-auto space-y-12">
                            <div className="grid grid-cols-2 gap-10">
                                            ...
                                            agents.map(a => (
                                                ...
                                                >
                                                    <div className={cn(
                                                        ...
                                                        formData.agent_id === a.id ? "bg-primary text-white border-primary" : "bg-card/40 border-border text-muted-foreground"
                                                    )}>
                                                        <Bot className="h-5 w-5" />
                                                    </div>
                                                    <div className="flex-1 min-w-0">
                                                        <h4 className="text-sm font-bold truncate">{a.name}</h4>
                                                        <p className="text-[10px] text-muted-foreground truncate font-medium">{a.description || "Sin descripción"}</p>
                                                    </div>
                                                    {formData.agent_id === a.id && (
                                        ...
                                        <div className="flex-1 space-y-2">
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <input 
                                                    ...
                                                    onChange={(e) => setFormData({...formData, bubble_color: e.target.value})}
                                                    title="Hex"
                                                    className="flex-1 h-10 bg-card/40 border border-border rounded-lg px-3 text-[10px] font-mono text-muted-foreground"
                                                />
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="space-y-8">
                                    ...
                                    <div className="bg-card/20 border border-border rounded-2xl p-6 space-y-4">
                                        <div className="flex items-center justify-between">
                                            <span className="text-[10px] font-black uppercase tracking-tight text-muted-foreground">Estado de Conexión Meta</span>
                                        ...
                                        <div className="space-y-2">
                                            <label className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/40">Plantilla de Handover</label>
                                            <select title="Plantilla" className="w-full h-10 bg-card/40 border border-border rounded-lg px-3 text-[10px] font-bold text-muted-foreground/60">
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40">Mensaje de Bienvenida</label>
                                        <textarea 
                                            value={formData.welcome_message || ""}
                                            onChange={(e) => setFormData({...formData, welcome_message: e.target.value})}
                                            title="Bienvenida"
                                            className="w-full h-32 bg-card/40 border border-border rounded-2xl p-4 text-sm font-medium text-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all resize-none"
                                        />
                                    </div>

                                    <SectionHeader icon={Code} title="4. Código de Inserción" />
                                    <div className="bg-card/40 rounded-[32px] p-8 border border-border space-y-4">
                                        <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest leading-relaxed">
                                            Copia este script antes de la etiqueta &lt;/body&gt; de tu sitio web.
                                        </p>
                                        <div className="relative group">
                                            <pre className="p-5 bg-card/20 border border-border rounded-2xl text-[10px] font-mono text-primary/60 overflow-x-auto whitespace-pre-wrap">
                                                {`<script src="${typeof window !== 'undefined' ? window.location.origin : ''}/api/widget/embed.js?id=${selectedWidget.id}" async></script>`}
                                            </pre>
                                            <button onClick={() => copyEmbedCode(selectedWidget.id)} title="Copiar" className="absolute top-4 right-4 h-10 w-10 rounded-xl bg-card/40 hover:bg-card/60 flex items-center justify-center transition-all"><Copy className="h-4 w-4" /></button>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <AnimatePresence>
                {isCreateModalOpen && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-background/90 backdrop-blur-xl" onClick={() => setIsCreateModalOpen(false)} />
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative w-full max-w-md bg-card border border-border rounded-[40px] p-10 space-y-8">
                            ...
                            <div className="flex gap-4 pt-4">
                                <button onClick={() => setIsCreateModalOpen(false)} className="flex-1 h-14 rounded-2xl bg-card/40 font-black uppercase tracking-widest text-[11px]">Cancelar</button>
...
function SectionHeader({ icon: Icon, title }: { icon: LucideIcon | React.ElementType, title: string }) {
    return (
        <div className="flex items-center gap-3">
            ...
            <h3 className="text-[12px] font-black uppercase tracking-[0.2em] text-muted-foreground/60">{title}</h3>
        </div>
    );
}
    ...
    return (
        <div className="space-y-2">
            ...
            <input 
                ...
                title={label}
                className="w-full h-12 bg-card/40 border border-border rounded-xl px-4 text-sm font-bold text-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all"
            />
        </div>
    );
}
