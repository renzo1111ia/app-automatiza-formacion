...
**dashboard-esden** — AI CRM + Workflow Orchestrator multi-tenant para academias formativas (sector formación, ES + Latam). Versión actual: **v0.0.0**.
...
- **Audiencia interna del repo**: equipo de desarrollo Esden.
...
| --- | --- | --- |
| **1 — Sprint 0** | Hotfixes de seguridad (4 vulnerabilidades RLS multi-tenant + tokens OAuth + Kong EOL) | Pendiente |
| **2 — Capa de datos** | Consolidación capa Supabase + Zod + Repository pattern + RLS hardening (sin ORM nuevo) | Pendiente |
| **3 — Adapter layer + 2 CRMs** | HubSpot adapter + Zoho adapter + UI admin de conexión (**MVP**) | Pendiente |
| **4 — Hardening** | Tests E2E, observabilidad, dashboards de costes | Pendiente |
| **5 — Post-release** | Google Sheets bidireccional + Salesforce + GoHighLevel + ActiveCampaign | Futuro |

**MVP Fase 3 = HubSpot + Zoho.** Sheets NO entra en MVP — está aplazado a Fase 5.
...
3. **No `git remote add origin <url-cliente>`** — el repo local NO se conecta al GitHub del cliente (`renzo1111ia/dashboard-esden`).
...
5. **Dependency Guard**: TODA nueva dependencia de producción debe pasar por el subagente `esden-agents:adr` antes de instalarse. El hook `esden-deps-guard.cjs` lo bloquea automáticamente si se intenta saltar.
...
1. `npm run typecheck` + `npm run lint` + `npm run build` + tests (vía subagente `esden-agents:testing`).
...
El orquestador es **`esden-agents:manager`** (Opus). Coordina 19 subagentes especializados (14 del patrón saas-agents adaptados a Esden + debugger, journal-writer, mcp-manager, code-simplifier, researcher, brainstormer de la config global).

Para delegar: `Task(subagent_type="esden-agents:database", prompt="...")`.
...
- ✅ `plugin:playwright:playwright` — browser tests Fase D.
- 🔜 (Fase E) MCP/skill Google Sheets — añadir cuando lleguemos a esa fase.
