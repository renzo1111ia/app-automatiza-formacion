# Phase 08 — Cierre Sprint 3 (SP-3-CLOSE-1..5)

## Context Links

- Sprint 2 cierre: `plans/260520-1342-sprint-2-capa-datos/plan.md` §Cierre
...
- **Descripción:** Tareas de cierre obligatorias del Sprint 3: auto tests, browser tests del flujo OAuth, test manual del dev, corrección de bugs, PR a `developer` y bump de versión a `v0.3.0`.
...
- Bugs cosméticos: documentar para Sprint 4 si no son bloqueantes
...
git commit -m "chore: bump version to v0.3.0 — Sprint 3 complete"

# 4. Push y crear PR
git push origin feature/sp-3-adapter-hubspot-zoho
gh pr create --title "Sprint 3: Adapter HubSpot + Zoho MVP (v0.3.0)" \
  --body "..."
  --base developer

# 5. (Con autorización explícita del usuario) Merge PR a developer
# 6. Crear rama para Sprint 4
git checkout developer
git pull
git checkout -b feature/sp-4-hardening
```

**NOTA:** El merge a `developer` y la creación de rama Sprint 4 requieren autorización explícita del usuario antes de ejecutar.
...
## Criterios globales de "DONE" Sprint 3
...
- `esden-agents:testing` — SP-3-CLOSE-1 + SP-3-CLOSE-2
- Dev humano — SP-3-CLOSE-3 (test manual)
- `esden-agents:code` — SP-3-CLOSE-4 bug fixes
- `esden-agents:manager` — SP-3-CLOSE-5 PR + versión

## Next Steps

- Post-cierre: notificar al roadmap-keeper para actualizar `plans/RoadMap.md` Sprint 3 con estimaciones reales + estado "Completado"
- Sprint 4: Hardening (tests E2E, observabilidad, dashboards de costes)
