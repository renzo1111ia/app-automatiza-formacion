# Phase 07 — Cierre de Sprint 1

## Context Links
- [plan.md](plan.md) — overview Sprint 1 (criterios de éxito + gates obligatorios)
...
**Agentes asignados:**
- SP-1-CLOSE-1: `esden-agents:testing`
- SP-1-CLOSE-2: `esden-agents:testing` + `esden-agents:uxui`
- SP-1-CLOSE-3: `esden-agents:manager` (interacción con humano)
- SP-1-CLOSE-4: `esden-agents:code` + `esden-agents:debugger` (subtareas dinámicas)
- SP-1-CLOSE-5: `esden-agents:git` + `esden-agents:deployment` + `esden-agents:productivity` + `esden-agents:roadmap-keeper`
...
- [ ] `esden-agents:help-docs-keeper` actualizó secciones de ayuda afectadas

## SP-1-CLOSE-1 — Auto test (1h 30min)

**Delegado a:** `esden-agents:testing`
...
Si hay errores en cualquier paso → no continuar a CLOSE-2. Delegar corrección a `esden-agents:code` y repetir CLOSE-1.

## SP-1-CLOSE-2 — Test E2C Local — Browser + WCAG 2.2 AA (2h 30min)

**Delegado a:** `esden-agents:testing` + `esden-agents:uxui`
...
- **Scope**: solo las páginas/flujos afectados por las fases del Sprint 1. Los findings de DA-5 (accesibilidad general) son Sprint 4 — no bloquean este sprint salvo que sean nuevos o agravados por el Sprint 1.

Reporte: screenshots de pasos clave + lista de findings de accesibilidad encontrados.

## SP-1-CLOSE-3 — Test Manual del Dev (1h)

**Delegado a:** `esden-agents:manager` (orquesta la entrega al humano)
...
2. Guía paso a paso:
   ...
   - Intentar cambiar cookie `esden-tenant-id` a UUID ajeno → debe no obtener datos ajenos.
...
**Delegado a:** `esden-agents:code` + `esden-agents:debugger`
...
**Delegado a:** `esden-agents:git` + `esden-agents:deployment` + `esden-agents:productivity` + `esden-agents:roadmap-keeper`

Secuencia exacta:

### 1. Verificación de gates (esden-agents:git)
- Verificar que todos los gates de pre-requisitos están en 🟢.
- Si algún gate no está verde → STOP. No proceder hasta resolverlo.

### 2. CHANGELOG.md (esden-agents:deployment — gatekeeper)
...
### 3. PR a developer (esden-agents:git)
- Crear PR: `feature/sp-1-sprint-1-hotfixes` → `developer`.
- Título: `feat: Sprint 1 — Hotfixes de seguridad (v0.1.0)`.
- Description: link al plan + lista de tareas + changelog.
- Confirmar con el dev antes de mergear (requiere aprobación humana).

### 4. Bump SemVer (esden-agents:git, tras merge)
- Tras merge aprobado: bump en `package.json` a `v0.1.0`.
- Commit: `chore: bump version to v0.1.0`.
- Tag: `git tag v0.1.0`.

### 5. Crear rama Sprint 2 (esden-agents:git)
- `git checkout -b feature/sp-2-capa-datos` desde `developer` (post-merge).

### 6. Actualizar RoadMap (esden-agents:roadmap-keeper)
- Marcar Sprint 1 como 🟢 COMPLETADA.
...
- RoadMap.md: Sprint 1 en 🟢, todas las tareas 1-01..1-24 en 🟢.
...
| SP-1-CLOSE-4 acumula muchos bugs → sprint se extiende | Media | Medio | Cap de 2h por bug antes de escalar; bugs cosméticos → deferir a Sprint 2 |
...
**Delegado a:** `esden-agents:deployment`
...
> **Esta tarea NO bloquea el cierre del Sprint 1.** El sprint se cierra cuando se mergea a `developer` y se hace el bump a v0.1.0. Esta tarea sí bloquea la promoción a `main`.

### Precondiciones
- Sprint 1 cerrado en `developer` (tag v0.1.0 creado).
- Rama `feature/sp-1-sprint-1-hotfixes` mergeada a `developer`.
- Autorización explícita del usuario (Renzo) para promover a `staging`.

### Secuencia

1. **`esden-agents:deployment`** crea PR: `developer` → `staging`.
...
→ Sprint 2 planificado: [feature/sp-2-capa-datos] — ver [RoadMap.md Fase 2](../RoadMap.md)
→ Tarea de arranque de Sprint 2: delegar a `esden-agents:planning` para detallar el plan de Fase 2 en `plans/YYMMDD-HHmm-sprint-2-capa-datos/`.
→ SP-1-CLOSE-5-bis se puede iniciar en paralelo con el arranque de Sprint 2 (no bloquea el trabajo de desarrollo en Fase 2).
