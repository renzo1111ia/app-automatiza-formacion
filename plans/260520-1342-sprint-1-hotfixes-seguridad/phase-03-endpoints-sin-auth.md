...
**Agentes:** `esden-agents:code` (implementación) + `esden-agents:api` (contratos y middleware)
...
- 1-09: Añadir guard condicional en `/api/test/orchestrator`. El endpoint se activa solo si el tenant tiene `tenants.test_orchestrator_enabled = true` AND webhook config completa. Tenant nuevo arranca con el endpoint cerrado (deny by default).
  ...
  - **GUARD TEMPORAL** — el endpoint sigue sin autenticación de sesión. Deuda técnica → Fase 4: eliminar definitivamente o migrar a admin con auth.
  - Añadir ítem en backlog Fase 4: "Eliminar `/api/test/orchestrator` o migrar a admin con auth completa".
...
DESPUÉS (Ph3 aplicada):
  ...
  /api/test/orchestrator → checkTenantGuard(test_orchestrator_enabled + webhook completo) → handler()
                        [AVISO: sin auth de sesión — guard temporal hasta Fase 4]
...
>
> **Deuda técnica (Fase 4)**: este endpoint sigue siendo accesible sin autenticación de sesión — solo gateado por configuración del tenant. Crear ítem en backlog Fase 4: _"Eliminar definitivamente `/api/test/orchestrator` o migrar a admin con auth completa"_.
...
2. En `src/app/api/test/orchestrator/route.ts`, añadir al inicio del handler:
   ```ts
   // Guard condicional — TEMPORAL hasta Fase 4
...
4. UI admin: añadir toggle en la pantalla de administración de tenants para activar/desactivar `test_orchestrator_enabled` por tenant. Agente: `esden-agents:uxui`.
...
- [ ] 1-09: UI admin — toggle activar/desactivar por tenant (`esden-agents:uxui`)
- [ ] 1-09: Verificar que no hay tests que esperen el endpoint sin guard
- [ ] 1-09: Documentar deuda técnica Fase 4 en backlog
...
| Endpoint 1-09 sigue sin auth de sesión — guard solo por config de tenant | Alta | Medio | **Riesgo residual aceptado temporalmente**. Crear ítem en backlog Fase 4 para eliminación definitiva. No promover a producción sin documentar este riesgo. |
...
- 1-10 implementa verificación provisional con `auth_user_id`. Cuando Fase B introduzca `tenant_members`, actualizar a verificación por roles.
