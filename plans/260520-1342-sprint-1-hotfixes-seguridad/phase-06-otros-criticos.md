...
**Agentes:** `esden-agents:code` (1-22, 1-23, 1-25) + `esden-agents:adr` (1-24, 1-26, Dependency Guard obligatorio) + `esden-agents:security` (verificación post-1-26)
...
- **DA-3-CVE-001**: `axios@1.14.0` tiene 15 CVEs conocidos. El más grave (SSRF CVSS 7.2) permite a un atacante que controla la URL redirigir peticiones axios a targets arbitrarios, incluyendo servicios internos. El upgrade debe pasar por `esden-agents:adr` (Dependency Guard) antes de instalarse.
...
- **DA-3-CVE-002**: `next@16.1.6` tiene 19 CVEs activos. Los más graves: SSRF via WebSocket upgrades (CVSS 8.6, GHSA-c4j6-fc7j-m34r) y middleware/proxy bypass via dynamic route param injection (CVSS 8.1, GHSA-492v-c6pp-mqqv). El middleware bypass anula directamente las protecciones de auth añadidas por 1-07, 1-08, 1-16 y 1-17 — sin este fix esas tareas son inefectivas. Movida de Sprint 2 (2-27) a Sprint 1 por impacto crítico en seguridad real.

## Requirements

### Funcionales
- 1-22: Implementar allowlist dinámica POR TENANT. Nueva columna `tenants.allowed_migrate_hosts text[]`. El endpoint valida el host de la cookie `esden-tenant-url` contra `tenants.allowed_migrate_hosts` del tenant activo. Si la lista está vacía → 403 (deny by default). UI admin para gestionar la lista de hosts por tenant.
...
- 1-24: Actualizar axios a la versión estable más reciente sin CVEs activos. Pasar por `esden-agents:adr` primero para validar compatibilidad y breaking changes.
...
- 1-26: Actualizar `next` a `16.2.6` + `eslint-config-next` a `16.2.6` (peer dep ligada). Pasar por `esden-agents:adr` primero. Smoke test de rutas críticas. Rollback inmediato si hay regresión. Marcar 2-27 en Sprint 2 como "movida a 1-26".
...
```
ANTES:
  cookie esden-tenant-url = "http://cualquier-url.com"
...
### 1-24 — Axios upgrade (vía esden-agents:adr)
...
Proceso:
  1. esden-agents:adr verifica compatibilidad y genera ADR
...
### 1-26 — Update next@16.1.6 → next@16.2.6 (vía esden-agents:adr)
...
Proceso:
  1. esden-agents:adr revisa changelog 16.1.6 → 16.2.6 y confirma no-breaking
  ...
  7. Marcar 2-27 en plan Sprint 2 como "MOVIDA A 1-26 (completada en Sprint 1)"
```
...
- `docs/adr/ADR-001-axios-upgrade.md` — decisión de arquitectura (generado por esden-agents:adr, 1-24)
- `docs/adr/ADR-002-next-upgrade-16-2-6.md` — decisión de arquitectura (generado por esden-agents:adr, 1-26)
...
2. Leer `api/tenant/migrate/route.ts:247-263` — entender cómo se obtiene el tenant activo y cómo se usa la cookie `esden-tenant-url`.
...
5. UI admin: pantalla/sección para que el admin del sistema añada/elimine hosts permitidos para cada tenant (`tenants.allowed_migrate_hosts`). Agente: `esden-agents:uxui`. Estimación UI: ~1h incluida en las 8h.

6. Verificar si la cookie `esden-tenant-url` es `httpOnly` y `Secure`. Si no → documentar como riesgo residual.
...
> **OBLIGATORIO**: Pasar por `esden-agents:adr` ANTES de instalar. El hook `esden-deps-guard.cjs` bloqueará si se intenta instalar sin ADR aprobado.

1. Delegar a `esden-agents:adr`:
...
7. Actualizar nota en plan Sprint 2: marcar 2-27 como "MOVIDA A 1-26 — completada en Sprint 1".
...
- [ ] 1-22: UI admin — gestión de hosts permitidos por tenant (`esden-agents:uxui`)
...
- [ ] 1-24: Delegar a `esden-agents:adr` — versión a instalar + ADR
...
- [ ] **1-26: Delegar a `esden-agents:adr` — changelog 16.1.6→16.2.6 + compatibilidad + ADR-002**
...
- [ ] 1-26: Marcar 2-27 en Sprint 2 como "MOVIDA A 1-26"
- [ ] Typecheck global: `npm run typecheck` → 0 errores

## Success Criteria

- Tenant sin `allowed_migrate_hosts` configurado → 403 (deny by default) (1-22).
- `curl` con URL arbitraria en `esden-tenant-url` (host no en lista del tenant) → 403 (SSRF bloqueado) (1-22).
...
Las siguientes vulnerabilidades se detectaron en el audit pero NO están en las 24 tareas del Sprint 1. Se proponen para evaluación antes de Sprint 2:
