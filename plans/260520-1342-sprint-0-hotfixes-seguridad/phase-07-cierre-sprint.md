...
1. `npm run typecheck` — **0 errores TypeScript** (gate duro).
2. `npm run lint` — **NO superar el baseline** documentado en `plans/reports/lint-baseline-20260521.md` (164 errores `no-explicit-any` pre-existentes, planificados para Sprint 1 — ver tarea `2-XX Lint cleanup baseline`).
   - Criterio: errores ≤ 164. Si sube, hay `any` nuevos introducidos en este sprint → bloquea cierre hasta arreglarlos.
   - El pre-push hook ya lintea archivos cambiados, así que nuevos `any` en el sprint deberían haber sido bloqueados en su push.
3. `npm run build` — build exitoso sin errores (warnings aceptables).
4. `npm test` — unit tests + integration tests.

**Por qué no exigimos lint a 0 errores aquí:** ver `plans/reports/lint-baseline-20260521.md`. Los 164 errores `any` están en código de producción (`src/lib/integrations/`, `src/lib/core/processors/`, `src/types/database.ts`, etc.) que **Sprint 1 va a refactorizar con Repository pattern + Zod**. Tiparlos antes de Sprint 1 = trabajo doble. Tiparlos en Sprint 1 = aprovechamos el refactor.

Reporte esperado:

- Número de tests: passed / failed / skipped.
- Coverage si está configurado.
- Conteo de lint vs baseline (`164 esperado, X real`).
