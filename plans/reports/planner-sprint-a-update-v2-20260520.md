# Reporte: Sprint A — Actualización v2 de decisiones del usuario

**Fecha:** 2026-05-20
**Planner:** planner (Sonnet)
**Plan actualizado:** `plans/260520-1342-sprint-a-hotfixes-seguridad/`
...
- Riesgo residual documentado: el endpoint sigue accesible sin autenticación de sesión — solo gateado por config. Creado ítem de deuda técnica en Fase D: "Eliminar `/api/test/orchestrator` o migrar a admin con auth".
- **Estimación: 30min → 2h** (+1h 30min columna + validación + UI admin).

### A-06 — Verificación worker BullMQ (phase-02)
- Añadido Step 1 obligatorio al arrancar Sprint A: grep en `worker.js` para detectar si usa `pg`/`postgres-js` directo o Supabase client.
