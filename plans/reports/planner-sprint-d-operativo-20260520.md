---
title: "Planner Report — Sprint D Hardening — Plan Operativo"
date: 2026-05-20
agent: planner (Sonnet)
sprint: D
---

# Planner Report — Sprint D Plan Operativo

## Resumen

Plan operativo completo del Sprint D (Hardening) creado. 7 archivos de fase + plan.md en `plans/260520-1342-sprint-d-hardening/`. 3 researcher reports generados. Total estimado: 100-132h desarrollo + 8h cierre = **108-140h total** (centro ~116h).
...
|---------|--------------|------|--------|
| `plan.md` | Overview Sprint D | — | — |
| `phase-01-e2e-tests-playwright.md` | D-01 + D-02 | 28-32h | esden-agents:testing |
| `phase-02-observabilidad-logging-metricas.md` | D-03 | 12-16h | esden-agents:code + deployment |
| `phase-03-dashboard-costes-llm.md` | D-04 | 16-22h | esden-agents:code + uxui |
| `phase-04-wcag-22-aa.md` | D-05 (24 findings) | 28-40h | esden-agents:uxui + code |
| `phase-05-hardening-headers-rate-limits.md` | D-06 | 10-14h | esden-agents:security + code |
| `phase-06-documentacion-release-v1.md` | D-07 | 6-8h | esden-agents:documentation |
| `phase-07-cierre-sprint.md` | SP-D-CLOSE-1..5 | 8h + bugs | esden-agents:testing |
...
|-----------|-----------|
| D-01 E2E Playwright | 28-32h |
| D-02 Coverage 80% | incluido en Ph1 (setup compartido) + ~8h tests adicionales |
| D-03 Observabilidad | 12-16h |
| D-04 Dashboard costes LLM | 16-22h |
| D-05 WCAG 2.2 AA | 28-40h |
| D-06 Hardening headers/RL | 10-14h |
| D-07 Release docs | 6-8h |
| SP-D-CLOSE | 8h + bugs |
| **Total** | **108-140h + bugs** |
...
3. D-01 y D-02 son parcialmente paralelizables — wall-clock < suma individual
...
```
Sprint C completado (v0.3.0) — PREREQUISITO GLOBAL
    │
...
| Sprint | Componente que reutiliza Sprint D |
|--------|----------------------------------|
| Sprint A (A-26: next@16.2.6) | CSP headers en Ph5 requieren next parcheado primero |
| Sprint A (A-12..A-15: webhook signatures) | Tests de seguridad RLS en Ph1 verifican estos fixes |
| Sprint B (B-14..B-18: Repository pattern) | Coverage targets de Ph1 incluyen estos repositorios |
| Sprint B (B-31..B-34: shadcn updates) | WCAG Ph4 usa shadcn Dialog (instalado/actualizado en B) |
| Sprint C (LangChain multi-LLM) | Dashboard LLM Ph3 usa el pipeline existente |
...
1. **DA-5-012 scope final:** ¿Se incluye el responsive de AIAgentInbox (esfuerzo L, 8-12h) en v1.0.0 o se pospone a Sprint E? Recomendación: incluir pero marcarlo como último a implementar; cortar si sprint va a >130h.
...
5. **Widget embed CSP (DA-3-004):** El CSP de Ph5 necesita excepción para `/api/widget/*` (frame-ancestors *). Si el XSS de DA-3-004 NO fue corregido en Sprint A, el CSP mitiga parcialmente pero no elimina el riesgo. ¿Estado de DA-3-004?
...
**Summary:** Plan operativo Sprint D creado. 7 fases, 7 phase files, 3 researcher reports. Total ~116h centro. Dependencias entre fases documentadas. Prerequisito: Sprint C completo. Decisión arquitectónica principal: maximizar reutilización de deps existentes (ioredis para rate limits, Recharts para LLM dashboard, shadcn Dialog para modales WCAG) — solo ~8 nuevas devDeps y ~4 nuevas prodDeps.
