...
effort: 10-14h
status: pending
agents: [esden-agents:security, esden-agents:code]
---
...
- Sprint 1: auth en endpoints de orquestación (DA-2-001) + webhooks signatures — ya completados
...
- **CSP headers:** `unsafe-inline` necesario para Tailwind v4 en MVP — aceptar compromiso; `strict-dynamic` en Sprint 5
- **IMPORTANTE:** CSP debe implementarse DESPUÉS de que Sprint 1 corrija el XSS en widget embed (DA-3-004). Si Sprint 1 no está completo, el CSP protege parcialmente
- Next.js 16 CVE GHSA-ffhc-5mcf-pf4q (XSS via CSP nonces) ya corregido por 1-26 — condición previa
- `npm audit en CI` previene regresión de CVEs en futuros PRs — complementa el hook `esden-deps-guard.cjs` ya activo
...
- [ ] Rate limiting en `middleware.ts` (no romper auth middleware existente de Sprint 1)
...
- `npm audit --audit-level=high` en CI pasa (sin High/Critical CVEs tras Sprint 1)
...
- Webhooks (`/api/webhooks/*`): el rate limit NO reemplaza la validación de firma HMAC (ya implementada en Sprint 1). Son capas complementarias

## Next Steps

- Lighthouse test de CSP headers como parte de Ph1 Playwright
- Sprint 5 puede migrar CSP a `strict-dynamic` eliminando `unsafe-inline` si se configura Tailwind para no generar inline styles
- Budget alerts (basado en llm_usage_logs de Ph3) pueden añadirse como endpoint protegido en Sprint 5
