...
effort: 12-16h
status: pending
agents: [esden-agents:code, esden-agents:deployment]
---
...
- OpenTelemetry completo (trazas distribuidas) aplazado a Sprint 5 — MVP usa Pino únicamente
...
Tabla llm_usage_logs:
  ...
  RLS: tenant_id = jwt.tenant_id (mismo patrón Sprint 1)
```
...
```
ADR check: pino@^9.x no está en proyecto. Pasar por esden-agents:adr.
```
```bash
...
export const logger = pino({
  ...
  base: { service: 'dashboard-esden', env: process.env.NODE_ENV },
...
- SENTRY_DSN en variables de entorno, nunca hardcodeado (lección Sprint 1)
