...
last_updated: 29-05-2026 ~Sprint 3 MERGEADO a developer PR #22 (v0.3.0-rc.1), conflicts resueltos con SPIKE Sheets
last_updated_by: Claude orquestador (29-05-2026 — resolución conflicts merge developer→sprint-3 + cierre sesión Sprint 3 oficial. Detalle: (1) SP-4-CLOSE-1 🟢 typecheck+lint 0+build+236/240 Vitest. (2) SP-4-CLOSE-1.5 🟢 PASS — 0 críticos/2 altos/2 medios/3 bajos. (3) SP-4-CLOSE-2 🟢 14/14 specs Sprint 3 + 59/61 suite completa. (4) Bloque 3.B → 🟢 Diferida backlog post-MVP (decisión Javi HP). (5) 4 nuevas filas BUG-SEC-01..04 en tabla detallada Fase 3. (6) Cuadro mando Sprint 3 actualizado: ⏱ Push ~21h 22min, Bloque 3.B diferida, Cierre 🟡 En Desarrollo con CLOSE-1/1.5/2 🟢. (7) phase-04-validacion-sprint-3.md hand-off rellenado con specs exactas + BUG-SEC + vars VPS + notas deploy. (8) README.md raíz actualizado. (9) Conflicts merge resueltos: next.config.ts adopta PR #21 turbopack hotfix + Sprint 3 Sentry webpack.treeshake, google/callback adopta Sprint 4 SPIKE Sheets (descarta refactor getAuthServiceRoleKey ya obsoleto por reescritura completa del flujo OAuth Sheets).)
...
sprint_3_progress: "✅ **CERRADO v0.3.0-rc.1** — Sprint 3 mergeado a developer 29-05-2026 vía PR #22 · 21 commits (LINT-ZERO 9 lotes + DEPRECATIONS-DEPLOY 4 commits + AUTH-RATELIMIT + SEC-PROACTIVE + RLM-TIMEOUT + BUG-3-01..13 + AWS-REMOVAL + SIDEBAR-UX + WCAG-08/09/10 + NEW-09/10/11/12 parciales + NEW-13 + NODE-22 + 4-03/06/07/08) · CLOSE-1 🟢 (typecheck+lint 0+build+236/240 Vitest) · CLOSE-1.5 🟢 PASS (0 críticos/2 altos pre-deploy/2 medios backlog/3 bajos) · CLOSE-2 🟢 (14/14 Sprint 3 specs + 59/61 suite) · ⏱ Real ~21h 30min vs estim ~112-144h (ratio −82%). 4 BUG-SEC abiertos (01/02 pre-deploy VPS, 03/04 backlog). Bloque 3.B UI → backlog post-MVP."
---
...
| Item                                           | Estado                         | Estim.        | ⏱ Push     | ⏱ Cierre | Notas                                                                                                      |
| ---------------------------------------------- | ------------------------------ | ------------- | ---------- | -------- | ---------------------------------------------------------------------------------------------------------- |
| **🚀 Sprint 3**                                | 🟡 En Desarrollo               | **~112-144h** | ~21h 22min | —        | **v0.3.0-rc.1** (release candidate) · … _ver nota↓_                                                        |
| ▸ Tareas de desarrollo (Fase 3)                | 🔵 Ph cerrada                  | 93-119h       | ~21h       | —        | 22/22 🔵 dev (todas pusheadas, sin merge a developer todavía) · … _ver nota↓_                              |
...
| ▸ Cierre Sprint 3 (SP-4-CLOSE-1..5)            | 🟡 En Desarrollo               | **7h + bugs** | ~22min     | —        | CLOSE-1 🟢 + CLOSE-1.5 🟢 PASS + CLOSE-2 🟢 14/14 specs. CLOSE-4/5 pendientes. … _ver nota↓_               |
...
| BUG-SEC-01                       | **IP Spoofing en rate-limit** (A07:2021 OWASP) — `extractClientIp()` lee X-Forwarded-For sin priorizar X-Real-IP | 30min               | 🔘 Pendiente                    | Detectado CLOSE-1.5 Security delta 29-05-2026. `src/lib/rate-limiter.ts` `extractClientIp()` lee X-Forwarded-For sin priorizar X-Real-IP. En traefik sobrescribe el header pero exposición directa al 8500 sí permite spoofing. Fix: priorizar X-Real-IP. **Resolver antes del primer deploy VPS con tráfico real**, no bloquea cierre Sprint 3.                                                                                                                                                                |
| BUG-SEC-02                       | **Webhook workflow sin autenticación** (A01:2021 OWASP) — endpoint público sin firma dispara orquestación        | 1h                  | 🔘 Pendiente                    | Detectado CLOSE-1.5. `src/app/api/webhooks/workflow/[workflowId]/[path]/[nodeId]/route.ts` sin HMAC signature. Pre-existente en developer, no introducido por Sprint 3. Fix: HMAC signature como webhooks Retell/CRM. **Resolver antes del primer deploy VPS con tráfico real**.                                                                                                                                                                                                                                |
| BUG-SEC-03                       | **Email en claro en logs** (A09:2021 OWASP) — PII GDPR en `auth.ts`                                              | 20min               | 🔘 Pendiente (backlog post-MVP) | Detectado CLOSE-1.5. `src/lib/actions/auth.ts` líneas 65/86/105 logean email en plano. Fix: hash con sha256-8b (mismo patrón que rate-limiter bucket).                                                                                                                                                                                                                                                                                                                                                          |
| BUG-SEC-04                       | **Fail-open silencioso WhatsApp pause check** (A04:2021 OWASP) — leads pausados pueden recibir mensajes          | 15min               | 🔘 Pendiente (backlog post-MVP) | Detectado CLOSE-1.5. `src/lib/integrations/whatsapp.ts:87` try-catch silencia error del check `is_ai_paused`. Riesgo: leads pausados pueden recibir mensajes si falla la BD. Fix: bloquear envío por defecto si el check falla (fail-closed).                                                                                                                                                                                                                                                                   |
