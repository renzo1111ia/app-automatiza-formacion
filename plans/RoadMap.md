...
| ▸ Cierre Sprint 0 (SP-1-CLOSE-1..5)       | 🟢 Completada          | 5h 30min + bugs | —        | —         | CLOSE-1 DONE_WITH_CONCERNS (typecheck+build OK, lint 128 err preexistentes) · CLOSE-2 ✅ 24/24 E2E + 2 bugs corregidos (BUG-001 logout, BUG-002 viewer→admin guard) · CLOSE-3 ✅ análisis cruzado docs Bea+Renzo V1 (output: 13 tareas NEW-01..NEW-13) · CLOSE-4 ✅ bugs cerrados · CLOSE-5 ✅ merged via PR #2                                                               |

### Sprint 1 — Capa de datos (sin ORM nuevo)

| Item                                                 | Estado                       | Estim.                                 | ⏱ Push | ⏱ Cierre | Notas                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ---------------------------------------------------- | ---------------------------- | -------------------------------------- | ------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **🚀 Sprint 1**                                      | 🟢 Completada (PR pendiente) | **~205h**                              | ~12h   | —        | v0.2.0 · `feature/sprint-01-capa-datos` (16 commits) · **CERRADO 22-05-2026 23:00** · ⏱ Push ~12h (dev + cierre en misma sesión, sin merge aún) · ⏱ Cierre se anotará al mergear PR a `developer`. 24 🟢 Completada + 8 🟢 Diferida + 6 ADRs (014-019). PR PENDIENTE por orden explícita del usuario. Resumen: [`plans/260520-1342-sprint-1-capa-datos/SP-2-CLOSE-summary.md`](260520-1342-sprint-1-capa-datos/SP-2-CLOSE-summary.md) |
| ▸ Bloque 2.1 — Unificación cliente Supabase          | 🟢 Completada parcial        | 19h                                    | —      | —        | 3/3 · **2-01 ✅** audit (reporte `sp-2-01-audit-clients-supabase-20260522.md`) · **2-02.a ✅** upgrade ssr 0.10.3 + supabase-js 2.106.1 (ADR-016) · **2-02.b ✅** DI services parcial (chat-memory, appointment-service, ai-analysis) · **2-03 ✅** 3 services cleanup. Resto cron/webhooks/processors/whatsapp.ts → ADR-019                                                                                                          |
| ▸ Bloque 2.2 — Schemas Zod                           | 🟢 Completada                | 25h                                    | —      | —        | 9/9 · 8 schemas + barrel en `src/lib/schemas/` (leads, tenants, programs, appointments, ai-agents, knowledge-base, integrations, opportunities) · **2-35** ModelNameSchema whitelist + drop hack widget.ts:150 + migración SQL `20260522210000`                                                                                                                                                                                       |
| ▸ Bloque 2.3 — Repository pattern                    | 🟢 Completada                | 31h                                    | —      | —        | 7/7 · 7 repositorios + barrel en `src/lib/repositories/` (patrón `IRepository<T>` + `RepoResult` + `withTenantFilter` + `paginate` + `handleSupabaseError`). Repos: leads, tenants, appointments(+calls+attempts), ai-agents(+variants+voice), knowledge-base(+embeddings+chat-messages), integrations(+field-mapping+write-audit+webhooks), lead-opportunities                                                                       |
| ▸ Bloque 2.4 — Refactor queries existentes           | 🟢 Diferida                  | 18h                                    | —      | —        | 0/3 desarrollo · **2-19, 2-20, 2-21 → ADR-019** migración incremental (código nuevo usa repos; 57 queries en actions/, 10 en api/, worker.js+processors a migrar incrementalmente) · ~~2-36~~ MOVIDA a Sprint Costes-LLM (C-03) post-MVP                                                                                                                                                                                              |
| ▸ Bloque 2.5 — Type safety + limpieza                | 🟢 Completada parcial        | 17h                                    | —      | —        | 1/2 · **2-37 ✅** logger estructurado `src/lib/utils/logger.ts` con scrubbing PII (reemplaza console.log widget) · **2-22 🟢 Diferida ADR-019** (426 `as any` baseline documentado, Sprint v0.5.4 candidate)                                                                                                                                                                                                                          |
| ▸ Bloque 2.6 — RLS hardening complementario          | 🟢 Completada                | 19h                                    | —      | —        | 4/4 · **2-23 ✅** RLS ai_agents+ai_agent_variants (F-04-005) · **2-24 ✅** RLS web_widgets (F-04-006) · **2-25 ✅** RLS programas (F-04-008) · **2-26 ✅** AES-256-GCM tokens OAuth (ADR-017, `token-crypto.ts` + tabla integrations + ENCRYPTION_KEY) · 2-27 → 1-26                                                                                                                                                                  |
| ▸ Bloque 2.7 — Testing + documentación               | 🟢 Completada parcial        | 16h                                    | —      | —        | 2/2 · **2-28** Vitest + 58 unit tests pasando + 4 integration skip-by-env (schemas 35, crypto 8, logger 4, base-repository 11, lead-opportunities 4). E2E full → SP-4B · **2-29 ✅** `docs/architecture/data-layer.md` sección 6 + SP-2-CLOSE-summary.md                                                                                                                                                                              |
| ▸ Bloque 2.8 — Hardening dependencias (ADR)          | 🟢 Completada parcial        | 20h                                    | —      | —        | 3/5 · **2-30 ✅** hook `af-productivity-logger.cjs` Path B híbrido · **2-33 ✅** @types/node ^20 → ^24.12.4 · **2-37 ✅** logger estructurado (compartida con 2.5) · **2-31, 2-32, 2-34 🟢 Diferida ADR-018** post-MVP v0.6.x (lucide-react, shadcn 4, eslint 10)                                                                                                                                                                     |
| ▸ **Bloque 2.9 — Fix bugs Renzo + reqs Bea (NUEVO)** | 🟢 Completada                | **23h** (8h→3h NEW-01 paso 3 diferido) | —      | —        | 4/4 · **NEW-01 paso 2 ✅** fix saveOrchestratorConfig (commit `837e12f`); paso 3 DIFERIDO a v0.5.3 post-MVP (ADR-015) · **NEW-02 ✅** enum unificado `LeadStageEnum` + UNREACHABLE + refactor 6 ficheros · **NEW-06 ✅** modelo oportunidades múltiples + dedup 48h (tabla `lead_opportunities`, repository, ingest) · **NEW-13 ✅** política handoff humano (ADR-014, `handoff.ts`, migración SQL)                                   |
| ▸ Cierre Sprint 1 (SP-2-CLOSE-1..5)                  | 🟢 Completada                | **4h 30min + bugs**                    | —      | —        | CLOSE-1 ✅ (typecheck OK / lint 120 err preexistentes baseline ADR-019 / build 41 rutas / 58 tests OK) · CLOSE-2 ✅ smoke crypto + hook (E2E full → SP-4B) · **CLOSE-3 🟢 Diferida** a SP-4B phase-02 bloque 4 · CLOSE-4 ✅ lint fixes (9f1fbca + ccd6a50) · CLOSE-5 ✅ hand-off phase-02 rellenado · PR a developer pendiente orden usuario                                                                                          |
...
| ▸ Cierre Sprint 2 (SP-3-CLOSE-1..5)            | 🔘           | **4h 30min + bugs** | —      | —        | Sigue [Protocolo estándar de cierre](#protocolo-estándar-de-cierre-de-sprint-close-1-5--paso-6-condicional) · CLOSE-3 **DIFERIDO a SP-4B phase-03 bloque 4** · paso 7 E2E VPS condicional (omitido hasta despliegue VPS) · paso 8 hand-off SP-4B obligatorio                                                                                                                |
...
| 👤 ▸ Cierre SP-4B (CLOSE-1..5)           | 🔘           | 6-12h + bugs | —      | —        | Sigue [Protocolo estándar de cierre](#protocolo-estándar-de-cierre-de-sprint-close-1-5--paso-6-condicional) · SP-4B absorbe CLOSE-3 manual de Sprints 1, 2, 2B, 3 (Renzo) · **paso 7 E2E VPS OBLIGATORIO** (es el sprint de validación VPS) · paso 8 N/A (es el propio destino del hand-off) · Re-run consolidado + bump SemVer **v0.4.0 GA** · NO promociona a staging (requiere orden Javi HP) |
...
| **Estado del sprint**          | 🟢 Completada (PR a `developer` pendiente por orden explícita del usuario)             |
...
| ID   | Tarea                                                                                  | Estimación | Estado                | Notas                                                                                                                                                                                   |
| ---- | -------------------------------------------------------------------------------------- | ---------- | --------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2-01 | Auditar TODOS los usos directos `pg` / `postgres` / `postgres-js` en `src/`            | 4h         | 🟢 Completada         | ⏱ Real: ~30min. Reporte `plans/reports/sp-2-01-audit-clients-supabase-20260522.md`                                                                                                      |
| 2-02 | Refactor: mover queries directas `pg`/`postgres` a `@supabase/ssr` (cliente unificado) | 12h        | 🟢 Completada parcial | 2-02.a upgrade Supabase ssr 0.10.3 + supabase-js 2.106.1 (ADR-016) ✅. 2-02.b refactor DI services parcial (chat-memory, appointment-service, ai-analysis ✅). Resto diferido a ADR-019 |
| 2-03 | Eliminar JWTs `service_role` residuales (los que sobrevivieron Sprint 0)               | 3h         | 🟢 Completada parcial | 3 services cleanup ✅. Resto en cron/webhooks/processors diferido a ADR-019                                                                                                             |

#### Bloque 2.2 — Schemas Zod

| ID   | Tarea                                                                                                                                              | Estimación | Estado        | Notas                                                                        |
| ---- | -------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | ------------- | ---------------------------------------------------------------------------- |
| 2-04 | Estructura `src/lib/schemas/` + base helpers Zod (uuid, timestamps, enums comunes)                                                                 | 4h         | 🟢 Completada | src/lib/schemas/\_base.ts + barrel                                           |
| 2-05 | Zod schemas: `leads` (cruzar con `VARIABLES DEFINIDAS` cliente)                                                                                    | 4h         | 🟢 Completada | src/lib/schemas/leads.ts                                                     |
| 2-06 | Zod schemas: `tenants` + `tenant_members`                                                                                                          | 2h         | 🟢 Completada | src/lib/schemas/tenants.ts                                                   |
| 2-07 | Zod schemas: `programs` / `courses`                                                                                                                | 2h         | 🟢 Completada | src/lib/schemas/programs.ts                                                  |
| 2-08 | Zod schemas: `appointments` + `calls`                                                                                                              | 3h         | 🟢 Completada | src/lib/schemas/appointments.ts                                              |
| 2-09 | Zod schemas: `ai_agents` / `ai_agent_variants` / `prompts`                                                                                         | 3h         | 🟢 Completada | src/lib/schemas/ai-agents.ts (ModelNameSchema 2-35)                          |
| 2-10 | Zod schemas: `knowledge_base` / `chat_memory` / `chat_summary`                                                                                     | 2h         | 🟢 Completada | src/lib/schemas/knowledge-base.ts                                            |
| 2-11 | Zod schemas: `integrations` / `webhooks` / `crm_field_mapping` / `crm_write_audit`                                                                 | 3h         | 🟢 Completada | src/lib/schemas/integrations.ts (prep Fase 2)                                |
| 2-35 | Zod `ai_agent_variants.model_name` whitelist (`z.enum([...])`) + migración de `gpt-4.1` huérfanos → `gpt-4o` + **eliminar parche `widget.ts:150`** | 2h         | 🟢 Completada | ModelNameSchema enforced. Parche widget.ts eliminado. Migración SQL aplicada |

#### Bloque 2.3 — Repository pattern

| ID   | Tarea                                                                       | Estimación | Estado　　　  | Notas                                                 |
| ---- | --------------------------------------------------------------------------- | ---------- | ------------- | ----------------------------------------------------- |
| 2-12 | Estructura `src/lib/repositories/` + interface base + helpers tenant-scoped | 4h         | 🟢 Completada | src/lib/repositories/\_base-repository.ts             |
| 2-13 | Repository: `leads`                                                         | 6h         | 🟢 Completada | leads-repository.ts con findByExternalId + softDelete |
| 2-14 | Repository: `tenants`                                                       | 4h         | 🟢 Completada | tenants-repository.ts                                 |
| 2-15 | Repository: `appointments` + `calls`                                        | 5h         | 🟢 Completada | appointments + Calls + Attempts repos                 |
| 2-16 | Repository: `ai_agents` (+ variants)                                        | 4h         | 🟢 Completada | ai-agents + variants + voice repos                    |
| 2-17 | Repository: `knowledge_base` + `chat_memory`                                | 5h         | 🟢 Completada | knowledge-base + embeddings + chat-messages repos     |
| 2-18 | Repository: `integrations` + webhooks                                       | 3h         | 🟢 Completada | integrations + field-mapping + write-audit + webhooks |

#### Bloque 2.4 — Refactor queries existentes (paralelizable)

| ID   | Tarea                                                                                                                                                                            | Estimación | Estado　　　 | Notas                                                     |
| ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | ------------ | --------------------------------------------------------- |
| 2-19 | Refactor: mover queries de `src/app/api/**/*.ts` a repositorios                                                                                                                  | 8h         | 🟢 Diferida  | ADR-019 migración incremental. Código nuevo usa repos     |
| 2-20 | Refactor: mover queries de server actions `src/lib/actions/` a repositorios                                                                                                      | 6h         | 🟢 Diferida  | ADR-019. 57 queries en actions/ a migrar incrementalmente |
| 2-21 | Refactor: mover queries de `worker.js` + processors a repositorios                                                                                                               | 4h         | 🟢 Diferida  | ADR-019. Worker + processors a migrar incrementalmente    |
| 2-36 | Persistir `token_usage` (`completion.usage`) en `chat_messages.metadata` para TODOS los consumidores OpenAI (WhatsAppAIProcessor, RescueWorker, **WidgetAction**, FactExtractor) | 2h         | 🟢 Diferida  | MOVIDA a Sprint Costes-LLM post-MVP                       |

#### Bloque 2.5 — Type safety y limpieza

| ID   | Tarea                                                                                                                                                          | Estimación | Estado        | Notas                                                    |
| ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | ------------- | -------------------------------------------------------- |
| 2-22 | Limpieza `as any` / `as unknown` — usar tipos derivados Zod via `z.infer`                                                                                      | 16h        | 🟢 Diferida   | ADR-019. 426 as any baseline. Sprint v0.5.4 candidate    |
| 2-37 | Reemplazar `console.log`/`console.error` con `widgetId`+`leadId`+payload en `widget.ts:30` y server actions críticas por logger estructurado con scrubbing PII | 1h         | 🟢 Completada | src/lib/utils/logger.ts scrubbing PII. widget.ts migrado |

#### Bloque 2.6 — RLS hardening complementario

| ID   | Tarea                                                                              | Estimación | Estado        | Notas                                                                      |
| ---- | ---------------------------------------------------------------------------------- | ---------- | ------------- | -------------------------------------------------------------------------- |
| 2-23 | Fix RLS `ai_agents` / `ai_agent_variants` tautológica (no filtra por tenant)       | 3h         | 🟢 Completada | Migración 20260522220000 ai_agents RLS owner_or_admin                      |
| 2-24 | Fix RLS `web_widgets` (devuelve todos los tenants)                                 | 2h         | 🟢 Completada | Migración 20260522220001 web_widgets RLS owner_or_admin                    |
| 2-25 | Fix `getPrograms` — añadir filtro tenant (expone programas de todos los clientes)  | 2h         | 🟢 Completada | Migración 20260522220002 programas RLS owner_or_admin                      |
| 2-26 | Cifrar Google OAuth tokens en JSONB (no plano)                                     | 12h        | 🟢 Completada | AES-256-GCM token-crypto.ts + tabla integrations + ENCRYPTION_KEY. ADR-017 |
...
| ID   | Tarea                                                                            | Estimación | Estado　　　          | Notas                                                             |
| ---- | -------------------------------------------------------------------------------- | ---------- | --------------------- | ----------------------------------------------------------------- |
| 2-28 | Tests de integración con BD real (NO mocks) para repositorios principales        | 12h        | 🟢 Completada parcial | Vitest + 58 unit tests + 4 integration skip-by-env. E2E SP-4B     |
| 2-29 | Documentar capa de datos en `docs/architecture/data-layer.md` (refresh completo) | 4h         | 🟢 Completada         | docs/architecture/data-layer.md sección 6 + SP-2-CLOSE-summary.md |

#### Bloque 2.8 — Hardening de dependencias (hallazgos ADR audit 2026-05-20)

| ID                               | Tarea                                                                                  | Estimación | Estado        | Notas                                                                                 |
| -------------------------------- | -------------------------------------------------------------------------------------- | ---------- | ------------- | ------------------------------------------------------------------------------------- |
| 2-30                             | Crear hook `af-productivity-logger.cjs` para automatizar tracking de tiempos           | 6h         | 🟢 Completada | Spike Path B + af-productivity-logger.cjs híbrido + hooks.json                        |
| 2-31                             | Update `lucide-react@0.575` → `lucide-react@1.x` (major — testing visual iconos)       | 4h         | 🟢 Diferida   | ADR-018 post-MVP v0.6.x                                                               |
| 2-32                             | Update `shadcn@3.x` → `shadcn@4.x` (major — revisar componentes y theme)               | 6h         | 🟢 Diferida   | ADR-018 post-MVP v0.6.x (shadcn 4 requiere Tailwind 4)                                |
| 2-33                             | Alinear `@types/node@^20` con runtime Node 24                                          | 2h         | 🟢 Completada | @types/node ^20 -> ^24.12.4                                                           |
| 2-34                             | Investigar update `eslint@9` → `eslint@10` (bloqueado por eslint-config-next peer dep) | 2h         | 🟢 Diferida   | ADR-018 research-only. Bloqueado por eslint-config-next                               |
...
| ID                           | Tarea                                                                                                                                   | Estimación          | Estado                | Notas                                                               |
| ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- | ------------------- | --------------------- | ------------------------------------------------------------------- |
| SP-2-CLOSE-1                 | Auto test                                                                                                                               | 1h 30min            | 🟢 Completada         | typecheck OK / lint baseline / build 41 rutas / 58 tests OK         |
| SP-2-CLOSE-2                 | Test E2C Local + WCAG 2.2 AA                                                                                                            | 2h 30min            | 🟢 Completada parcial | Smoke tests crypto + hook OK. E2E SP-4B                             |
...
| SP-2-CLOSE-4                 | Corrección de Bugs detectados                                                                                                           | (variable)          | 🟢 Completada         | Lint fixes aplicados (commit 9f1fbca + ccd6a50)                     |
| SP-2-CLOSE-5                 | Cierre de Sprint → PR a `developer` + bump a `v0.2.0` + crear rama Sprint 2 + **hand-off a SP-4B phase-02** (rellenar plantilla)        | 1h                  | 🟢 Completada         | Hand-off phase-02 rellenado. PR a developer pendiente orden usuario |
...
| **Sprint 1** (Fase 1)            | v0.2.0           | 🟢 Completada (PR pendiente) | **32** (24 🟢 Completada + 8 🟢 Diferida; 1 movida a Sprint 0 + 1 a Sprint Costes-LLM) | ~205h estim · ⏱ Real ~12h                             | 5h 30min + bugs |
