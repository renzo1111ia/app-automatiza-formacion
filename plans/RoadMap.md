...
last_updated: 24-05-2026 23:30
last_updated_by: Javi HP (25-05-2026 ~13:30 UTC — Sprint 2B POST-FIX v0.2.9: tras cierre v0.2.8 el usuario detectó visualmente 2 bugs en VPS (alturas reales cards distintas 382/362/362 + max-w-[1600px] limitando viewport en pantallas grandes). Corregidos en 3 commits (4c720e1 alturas uniformes auto-rows-fr, 7cfc976 alturas reales DOM h-full + card replicada canal distribution, 0fa1cfc viewport w-full pantallas grandes). E2E manual Bloques B-G ejecutado (32/43 PASS, 0 FAIL críticos). 4 bugs detectados: BUG-2B-08/09/10 WCAG menores diferidos Sprint 3 + BUG-2B-11 viewport FIXED y deployed VPS. Bump v0.2.8→v0.2.9, tag + release pendientes. Suite Playwright 18/18 verde local + VPS)
...
| Deadline MVP                      | **"Lo antes posible"** — objetivo **Lun 22-06-2026** (v0.3.0 GA, adelantado −7 sem por ratio real Sprints 0/1/2 −91% a −94% vs estim · decisión 24-05-2026 21:30). Sin compromiso externo de fecha con cliente.                                                                                                                                                                                                                   |
...
| ▸ Phase 02 Zoho multi-DC bugfixes (B-01..B-07) | 🟢 Completada     | 10h → ~10min                     | ~10min | ~10min    | Commit `74cc137` mergeado en `a826fd6`. `zoho-dc-detector.ts` (9 DCs + exchangeCodeForTokens + refreshAccessToken). Refactor `zoho.ts` (B-01..B-07 fix). Tests: 13 + 5 (dc-detector). MSW handlers en `tests/mocks/zoho-handlers.ts`.                                                                                                                                                                                                                                                                                                                                                                                                 |
| ▸ Phase 03 HubSpot Public App OAuth            | 🟢 Completada     | 16h → ~8min                      | ~8min  | ~8min     | Commit `74cc137` mergeado en `a826fd6`. `hubspot.ts` provider Public App OAuth + fetch puro. CRUD contacts/tasks/meetings con association IDs. Tests: 21 + 7 (mappers). Doc `docs/integrations/hubspot-app-setup.md`. **Bloqueante usuario** (pre-deploy real, no bloquea release): registrar Public App en developers.hubspot.com.                                                                                                                                                                                                                                                                                                   |
| ▸ Phase 04 WriteGuard + audit log              | 🟢 Completada     | 6h → ~4min                       | ~4min  | ~4min     | Commit `74cc137` + `ce166ec` (F-WG-1 fail-closed) mergeados. `write-guard.ts` con `applyWritePolicy` standalone + escape hatch `allowEmptyCurrent`. `audit-query.ts` getAuditLog RLS multi-tenant. Tests: 10 unit + 2 integration.                                                                                                                                                                                                                                                                                                                                                                                                    |
| ▸ Phase 05 UI admin IntegrationsManager        | 🟢 Completada     | 12h → ~10min + 30min (hotfix)    | ~10min | ~40min    | Commits `74cc137` + `ce166ec` mergeados. **HOTFIX BUG-2-01 (commit `9ace75f`)**: routes `[id]/*` movidos a `manage/[id]/*` por slug conflict con `[provider]/*` en Next.js App Router. 4 fetch frontend actualizados. UI: crm-section + crm-provider-card + write-policy-editor + audit-log-viewer. Tests: 6 API oauth-callback. **WCAG axe + Playwright E2E completos** diferidos a Sprint 3.                                                                                                                                                                                                                                        |
| ▸ Phase 06 Tests + docs + ADRs 021/022/023     | 🟢 Completada     | 10h → ~5min                      | ~5min  | ~5min     | Commits `74cc137` + `ce166ec` (F-COV-1 coverage config) mergeados. `docs/architecture/crm-adapters.md` 10 secciones. ADR-021/022/023. Migración help_sections seed. **Tests: 170 passed + 4 skipped.**                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ▸ Phase 07 Cierre (SP-3-CLOSE-1..5)            | 🟢 Completada     | 6h + bugs → ~3h 15min            | ~5min  | ~3h 15min | CLOSE-1 🟢 typecheck + build + 170 tests + lint diff. CLOSE-2 🟡 parcial local + **5/5 E2E VPS verdes** post-hotfix. CLOSE-3 🟢 diferida SP-4B. CLOSE-4 🟢 BUG-2-01 fix (`9ace75f`) + lint warnings fix (`107cd7a`) + lint-staged downgrade (`c426bfb`). CLOSE-5 🟢 PR #12 mergeado (`a826fd6`) + bumps v0.2.5/v0.2.6/v0.2.7 + tags + releases + Node 22 planning Sprint 3 (`e1f4af0`) + release notes v0.2.7 (`1f3cae1`). QA report: `plans/reports/qa-260524-1625-sprint-2-cierre.md`.                                                                                                                                              |
...
| Item                                  | Estado       | Estim.       | ⏱ Push | ⏱ Cierre | Notas                                                                                                                                                                                                                                                                                                                                                                                            |
| ------------------------------------- | ------------ | ------------ | ------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 👤 **🚀 Sprint Validación Pre-MVP**   | 🔘 Pendiente | **40-55h**   | —      | —        | **v0.3.0 GA** · `feature/sprint-03b-validacion-pre-mvp` · Inicio Mar 16-06-2026 (post-Sprint 3 merge · adelantado −7 sem 24-05) · **Asignado a Renzo + equipo Renzo** · estim. recalculada 22-05-2026 por absorber CLOSE-3 manuales de Sprints 1, 2, 2B, 3                                                                                                                                       |
...
| 👤 ▸ Phase 03a — Validación Sprint 2  | 📝 Llenada   | 5-7h         | —      | —        | Rellenada 24-05-2026 al cierre Sprint 2 (v0.2.7) tras research R3. Incluye **BUG-2-01** (P0 slug conflict) + F-WG-1 + F-API-1 + F-API-2 como regression baseline. Versión corregida `v0.2.0-rc` → `v0.2.7`. Decisión R3: dividida de phase-03 antigua para no mezclar bug IDs ni branches con Sprint 2B                                                                                          |
| 👤 ▸ Phase 03b — Validación Sprint 2B | 🔘 Skeleton  | 5-6h         | —      | —        | Skeleton creado 24-05-2026 con placeholders `<!-- AUTOFILL -->`. Cubre Sprint 2B Dashboard KPIs Overview (v0.2.8). Se rellena en SP-3B-CLOSE-5 (auto-fill)                                                                                                                                                                                                                                       |
| 👤 ▸ Phase 04 — Validación Sprint 3   | 🔘 Skeleton  | 10-12h       | —      | —        | Skeleton expandido 24-05-2026 (70 líneas → 270+) con 5 subdominios estructurados: Node 22, E2E Playwright, Observabilidad Pino+Sentry+BullMQ, WCAG 2.2 AA, Headers+Rate-limits, + NEW-09..12. Env vars probables ya listadas (`SENTRY_DSN`, `PINO_LOG_LEVEL`, `BULL_BOARD_*`). Se auto-fill en SP-4-CLOSE-5                                                                                      |
...
| Item                                | Estado       | Estim.          | ⏱ Push | ⏱ Cierre | Notas                                                                                                                                                                                                              |
| ----------------------------------- | ------------ | --------------- | ------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **🚀 Sprint 4**                     | 🔘 Pendiente | 60-100h         | —      | —        | v0.5.0 · `feature/sprint-04-google-sheets` · Inicio Mar 23-06-2026 (post-MVP, adelantado −7 sem 24-05 aplicando ratio real Sprints 0/1/2)                                                                          |
...
| Item                                          | Estado       | Estim.          | ⏱ Push | ⏱ Cierre | Notas                                                                                                                                                                                                              |
| --------------------------------------------- | ------------ | --------------- | ------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **🚀 Sprint Costes-LLM**                      | 🔘 Pendiente | 23-31h          | —      | —        | v0.5.1 · `feature/sprint-costes-llm-post-mvp` · Inicio Lun 13-07-2026 (post-Sheets v0.5.0 · adelantado −7 sem 24-05) · creado 22-05-2026 por decisión clienta                                                      |
...
| **Rama de trabajo**            | `feature/sprint-02-adapter-hubspot-zoho` (pusheada + PR #12 mergeado `a826fd6`)                                      |
...
| ------------------------------ | ------------------------------------------------------------------------------------------------ | ------- | ------------- | ------ | -------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| **3.0** Setup                  | env vars (OAUTH*STATE_SECRET, HUBSPOT*\_, ZOHO\_\_) + carpetas + msw devDep                      | 2h      | 🟢 Completada | ~1min  | ~5min    | [phase-00](260524-1330-sprint-2-adapter-hubspot-zoho/phase-00-setup.md)                                                                      |
| **3.1** Foundation             | ICRMProvider ampliado + migration integrations + TokenManager dedup + crm-error + oauth-state    | 14h     | 🟢 Completada | ~8min  | ~37min   | [phase-01](260524-1330-sprint-2-adapter-hubspot-zoho/phase-01-foundation-interface-integrations-table.md)                                    |
| **3.2** Zoho multi-DC bugfixes | Fix B-01..B-07: api_domain dinámico, accounts-server, 401→refresh→retry, paginación, email exact | 10h     | 🟢 Completada | ~5min  | ~26min   | [phase-02](260524-1330-sprint-2-adapter-hubspot-zoho/phase-02-zoho-multidc-bugfixes.md)                                                      |
| **3.3** HubSpot Public App     | OAuth start+callback + adapter completo (CRUD + tasks + meetings) + custom properties af\_\*     | 16h     | 🟢 Completada | ~9min  | ~42min   | [phase-03](260524-1330-sprint-2-adapter-hubspot-zoho/phase-03-hubspot-public-app-oauth.md)                                                   |
| **3.4** WriteGuard + audit     | WriteGuard standalone + filtro append_only + crm_write_audit append-only RLS + tests             | 6h      | 🟢 Completada | ~3min  | ~16min   | [phase-04](260524-1330-sprint-2-adapter-hubspot-zoho/phase-04-write-guard-audit-log.md)                                                      |
| **3.5** UI admin               | IntegrationsManager refactor: cards CRM + OAuth flow + write_policy editor + audit viewer + WCAG | 12h     | 🟢 Completada | ~7min  | ~32min   | [phase-05](260524-1330-sprint-2-adapter-hubspot-zoho/phase-05-ui-admin-integrations.md)                                                      |
| **3.6** Tests + docs + ADRs    | Coverage Vitest ≥80% + crm-adapters.md + ADRs 020/021/022 + help_sections "integrations"         | 10h     | 🟢 Completada | ~5min  | ~26min   | [phase-06](260524-1330-sprint-2-adapter-hubspot-zoho/phase-06-tests-coverage-docs.md)                                                        |
| **3.7** Cierre                 | CLOSE-1..5 + hand-off SP-4B phase-03                                                             | 6h      | 🟢 Completada | ~3min  | ~16min   | [phase-07](260524-1330-sprint-2-adapter-hubspot-zoho/phase-07-sprint-close.md)                                                               |
...
| ---------------------------- | ------------------------------------------------------------------------------------------- | ------------- | ----------------------- | ---------- | ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| SP-3-CLOSE-1                 | Auto test (typecheck + lint + build + Vitest)                                               | 1h 30min      | 🟢 Completada           | ~5min      | ~5min         | 170 tests passed + 4 skipped. Cobertura ≥80% en `crm/`. Build VPS Dokploy ✓ Compiled successfully (sin warnings EBADENGINE tras downgrade lint-staged)                                                                                                                                                                                                     |
| SP-3-CLOSE-2                 | E2C Local + WCAG 2.2 AA (Playwright + axe-core)                                             | 2h 30min      | 🟢 Completada (parcial) | ~5min      | ~45min        | **5/5 E2E VPS smoke verdes** contra `dev.automatizaformacion.com` v0.2.7 (`tests/e2e/sprint-2-close/smoke-crm-vps.spec.ts`): redirect, login admin, /dashboard/settings, CRMSection con HubSpot+Zoho, GET /api/integrations 401. WCAG axe + Playwright E2E completos diferidos a Sprint 3                                                                  |
| ~~SP-3-CLOSE-3~~             | ~~Test Manual del Dev~~ — **DIFERIDO a 👤 SP-4B phase-03 bloque 4** (Renzo + equipo)        | (0h aquí)     | 🟢 Diferida             | —          | —             | Decisión 22-05-2026                                                                                                                                                                                                                                                                                                                                        |
| SP-3-CLOSE-4                 | Corrección de Bugs detectados                                                               | (variable)    | 🟢 Completada           | ~5min      | ~1h 40min     | **BUG-2-01 (P0)** slug conflict `[id]` vs `[provider]` en `/api/integrations/` → 500 global VPS. Fix: mover `[id]/*` a `manage/[id]/*`. Commits `9ace75f` + `107cd7a` (lint warnings unblock push) + `c426bfb` (downgrade lint-staged 17→16 + engines Node 20 para alinear local↔VPS)                                                                      |
| SP-3-CLOSE-5                 | Cierre → PR + bumps + tags + releases + hand-off SP-4B phase-03 + Node 22 planning Sprint 3 | 1h            | 🟢 Completada           | ~5min      | ~40min        | PR #12 mergeado `a826fd6`. Bumps: v0.2.5 → v0.2.7 (hotfix). Tags v0.2.5 + v0.2.7 publicados. Releases con notas profesionales completas. Node 22 LTS migration planificada en `phase-03-migracion-node-22-lts.md` Sprint 3 (4h-6h). `.nvmrc` pinned a 20.20.2. Hand-off SP-4B phase-03 pendiente (rellenar plantilla con specs E2E VPS + checklist manual) |
...
| ------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Sprint ID**                  | `SP-3B`                                                                                                                                                                                                                                                    |
| **Versión objetivo al cierre** | `v0.2.8` inicial + `v0.2.9` post-fix (alturas reales + viewport 100% + E2E manual Bloques B-G)                                                                                                                                                             |
| **Estado del sprint**          | 🟢 **CERRADO v0.2.9** 25-05-2026 ~13:30 UTC (post-fix de v0.2.8 cerrado a las 08:15 UTC) · 7 commits totales · 18/18 Playwright · E2E manual Bloques B-G 32/43 PASS · 4 bugs detectados (1 fixed + 3 WCAG diferidos Sprint 3) · hand-off SP-4B auto-filled |
| **Estimación total**           | **16-24h** desarrollo + 4h 30min cierre                                                                                                                                                                                                                    |
| **Rama de trabajo**            | `feature/sprint-02b-dashboard-kpis-conjunto` (pusheada 24-05-2026 ~22:30)                                                                                                                                                                                  |
...
| **Plan detallado**             | [`plans/260522-1800-sprint-2b-dashboard-kpis-conjunto/`](260522-1800-sprint-2b-dashboard-kpis-conjunto/plan.md) (7 archivos)                                                                                                                               |
...
| **Phase 07** Cierre                | CLOSE-5: PR a developer + bump v0.2.8 + hand-off SP-4B phase-03b                             | 4h 30min  | 🟢 Completada | 15min     | ~30min    | PR #13                        | ✅ CERRADO. PR #13 mergeado `17b2902` · bump v0.2.8 · RELEASE-NOTES publicadas · hand-off SP-4B auto-filled                                                                                                                                                                                                                                                                                                                                                                                                                              |
| **E2E VPS**                        | Playwright contra dev.automatizaformacion.com post-merge                                     | —         | 🟢 Completada | ~15min    | ~15min    | —                             | **15/15 specs verdes** (25-05 06:09 UTC, 1m 30s). Dokploy deploy verde. Push = Cierre (ejecución única post-merge)                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **Phase 07B** Post-fix v0.2.9      | Fix alturas reales cards Overview + viewport 100% pantallas grandes + E2E manual Bloques B-G | ~3h       | 🟢 Completada | ~30min    | ~3h 30min | `4c720e1` `7cfc976` `0fa1cfc` | Bump v0.2.9. 3 fixes: alturas uniformes (4c720e1), alturas reales DOM tras Bloque A2 visual feedback usuario (7cfc976), viewport w-full pantallas >1920px (0fa1cfc). E2E manual Bloques B-G ejecutado 25-05 ~12:30 UTC (32/43 PASS, 0 FAIL críticos, 4 bugs detectados BUG-2B-08/09/10 WCAG diferidos Sprint 3 + BUG-2B-11 FIXED y desplegado). Plan: `phase-07b-e2e-manual-bloques-b-g.md`. Suite Playwright sprint-2b-close 18/18 verde local + VPS. Decisión bump v0.2.9 SÍ (mejoras visuales significativas validadas en producción) |
...
| SP-3B-CLOSE-5                 | Cierre: PR a developer + bump v0.2.8 + hand-off SP-4B phase-03b                                        | 1h                  | 🟢 Completada | 15min  | ~30min    | ✅ CERRADO v0.2.8. PR #13 mergeado `17b2902` · 15/15 E2E VPS verdes · RELEASE-NOTES publicadas · hand-off SP-4B auto-filled                                                                                                                    |
| SP-3B-CLOSE-6 (post-fix)      | Post-fix v0.2.9: 3 commits fix alturas+viewport + E2E manual Bloques B-G + bump v0.2.9 + tag + release | ~3h                 | 🟢 Completada | ~30min | ~3h 30min | ✅ CERRADO v0.2.9. Commits `4c720e1` `7cfc976` `0fa1cfc` · 18/18 Playwright local + VPS · E2E manual B-G 32/43 PASS · 4 bugs detectados (3 WCAG diferidos Sprint 3 + BUG-2B-11 FIXED y desplegado) · Plan: phase-07b-e2e-manual-bloques-b-g.md |
...
| SP-4-NODE-22                     | Migración runtime Node 20 → 22 LTS (Dockerfile + `.nvmrc` + `engines` + desbloquear lint-staged 17). Alinea local ↔ VPS y elimina warnings EBADENGINE.                                            | 4h 00min – 6h 00min | 🔘 Pendiente  | phase-03 Sprint 3 (NUEVA 24-05-2026). Decisión Javi HP tras Sprint 2 hotfix v0.2.7. Detalle: `plans/260520-1342-sprint-3-hardening/phase-03-migracion-node-22-lts.md`. NO confundir con la antigua phase-03 (Dashboard costes LLM) movida al Sprint Costes-LLM post-MVP                                                                                                                                                                                                                                             |
...
| 4-05                             | Refactor accesibilidad WCAG 2.2 AA en todo el admin panel                                                                                                                                         | 28-40h              | 🔘 Pendiente  | phase-04 Sprint 3. Findings deep audit DA-5 (24 findings) + **3 BUG-2B nuevos detectados E2E Bloques B-G 25-05** (sub-tareas SP-4-WCAG-08/09/10 abajo)                                                                                                                                                                                                                                                                                                                                                              |
| SP-4-WCAG-08                     | **BUG-2B-08** (LOW) — Añadir aria-label/title a 3 botones "Personalizar" (Personalizar Overview Gráficos, Personalizar Embudo, Personalizar Gráficos) en SummaryManager + ChartManager            | 30min               | 🔘 Pendiente  | phase-04 Sprint 3 (sub-4-05). Detectado E2E Bloque C-03 25-05-2026. Mejora screen readers (WCAG SC 2.4.4 Link Purpose - ya cumple por texto pero refuerza accesibilidad)                                                                                                                                                                                                                                                                                                                                            |
| SP-4-WCAG-09                     | **BUG-2B-09** (MEDIUM) — Heading hierarchy: dashboard tiene 3× h1 (Automatiza Formación, Métricas Generales, Análisis Visual) + salto h1→h3. Bajar a h2 las secciones internas, mantener h1 único | 1h 30min            | 🔘 Pendiente  | phase-04 Sprint 3 (sub-4-05). Detectado E2E Bloque C-07 25-05-2026. WCAG SC 1.3.1 (estructura semántica). Impacta screen readers + SEO. Tocar `app/dashboard/page.tsx` + ChartManager (h1 "Análisis Visual" → h2) + SummaryManager (h1 "Métricas Generales" → h2)                                                                                                                                                                                                                                                   |
| SP-4-WCAG-10                     | **BUG-2B-10** (MEDIUM) — Añadir skip-link "Saltar al contenido principal" al inicio del `<body>` (sr-only por defecto, visible on focus) + `<main id="main-content">`                             | 1h                  | 🔘 Pendiente  | phase-04 Sprint 3 (sub-4-05). Detectado E2E Bloque C-06 25-05-2026. WCAG SC 2.4.1 Bypass Blocks. Componente nuevo `SkipLink.tsx` al inicio del layout, sidebar tab-trap evita actualmente saltarse la navegación                                                                                                                                                                                                                                                                                                    |
...
| SP-4-NEW-13                      | Endpoints `/api/health` + `/api/version` (sin auth, Edge runtime) — verificación post-deploy fiable + uptime monitoring + debug producción                                                        | 30min – 1h          | 🔘 Pendiente  | phase-02 Sprint 3 (observabilidad). Detalle: `plans/260520-1342-sprint-3-hardening/phase-02-observabilidad-endpoints-health-version.md`. Resuelve fricción detectada SP-3B-CLOSE-5 (25-05-2026): ETag opaco de Next.js impedía verificar si VPS sirvió build nuevo                                                                                                                                                                                                                                                  |
| SP-4-NEW-11                      | Rename UI Historial → Leads + consolidación menús/labels/breadcrumbs/rutas. HACER ANTES de phase-01 E2E para evitar specs Playwright con nombres viejos                                           | 2h                  | 🟢 Completada | ⏱ Push: ~15min · ⏱ Cierre: ~15min · 3 labels UI cambiados (Sidebar submenu Métricas:132 'Historial'→'Tabla Leads' / page.tsx:38 h1 'Historial de Llamadas'→'Leads' / HistorialColumnManager:213 texto descriptivo). Preservados por semántica distinta: DuplicateLeadDialog 'Historial del número' (modal por teléfono) + voice-agents tab 'Historial' (de un agente específico). Approach B aplicado (URLs `/dashboard/historial` intactas, ids técnicos preservados). Typecheck 🟢. Approach validado con Javi HP |
| **Subtotal Fase 3 — Desarrollo** |                                                                                                                                                                                                   | **98-129h**         |               | Objetivo base 99h (+4h por SP-4-NODE-22 + 30min-1h por SP-4-NEW-13 + 3h por SP-4-WCAG-08/09/10 + 2h por SP-4-NEW-11 desglosada como fila individual). + 8h cierre phase-07. Total 106-137h con cierre. Resto Bloque 3.B (NEW-09/10/12) sigue en fila resumen agregada arriba (líneas 211-212)                                                                                                                                                                                                                       |
