...
sprint_2_progress: "🟡 EN DESARROLLO desde 24-05-2026 14:00 · branch `feature/sprint-02-adapter-hubspot-zoho` · **AUTOEXEC 24-05-2026 ~15:00→15:40**: Phase 02-07 código + tests + docs + ADRs cerrados en commit local `74cc137`. 168 tests Vitest verdes (vs 105 checkpoint Phase 01, +63). Typecheck + build verdes. 0 lint errors nuevos. Doc `crm-adapters.md` + ADR-021/022/023. Hand-off SP-4B/phase-03 actualizado. PR-BODY redactado en `plans/260524-1330-sprint-2-adapter-hubspot-zoho/PR-BODY.md`. **CLOSE-5 pendiente orden usuario:** `git push` + `gh pr create --base developer` (NO merge). **Bloqueante pre-deploy:** registrar HubSpot Public App en developers.hubspot.com + generar `OAUTH_STATE_SECRET` + aplicar migración `20260524110000_help_sections_integrations.sql` al VPS."
...
| Item                                           | Estado               | Estim.                           | ⏱ Push | ⏱ Cierre | Notas                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ---------------------------------------------- | -------------------- | -------------------------------- | ------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **🚀 Sprint 2**                                | 🟡 **En Desarrollo** | **74h** (refinada vs ~169h orig) | —      | —        | v0.3.0 · `feature/sprint-02-adapter-hubspot-zoho` · **Inicio REAL: 24-05-2026 14:00** · Estim refinada tras 3 researchers (HubSpot v3 fetch puro, Zoho multi-DC, adapter pattern) — Sprint 1 ya entregó token-crypto + integrations table base. [Plan completo](260524-1330-sprint-2-adapter-hubspot-zoho/plan.md) · 8 fases (00-07). **CHECKPOINT 24-05-2026 ~15:00: Phase 00+01 🟢 cerradas (commits `38a6667` + `000cd23`).** **AUTOEXEC 24-05-2026 ~15:00→15:40: Phase 02-07 código + tests + docs + ADRs completos en commit local `74cc137`.** Push + PR a `developer` pendientes orden usuario. |
...
| ▸ Phase 02 Zoho multi-DC bugfixes (B-01..B-07) | 🟡 Código listo      | 10h → ~10min                     | —      | —        | Commit local `74cc137` (push pendiente). `zoho-dc-detector.ts` (9 DCs + exchangeCodeForTokens + refreshAccessToken). Refactor `zoho.ts` (B-01..B-07 fix: paths v8, 401 retry, 429 backoff, 5xx exp backoff, paginación, módulo configurable, email exact). Tests: 13 (B-01..B-07 + 429 + 5xx) + 5 (dc-detector). MSW handlers en `tests/mocks/zoho-handlers.ts`.                                                                                                                                                                                                                                       |
| ▸ Phase 03 HubSpot Public App OAuth            | 🟡 Código listo      | 16h → ~8min                      | —      | —        | Commit local `74cc137` (push pendiente). `hubspot.ts` provider Public App OAuth multi-tenant + fetch puro. `hubspot-mappers.ts` (VARIABLES DEFINIDAS ↔ HubSpot, truncate af_metadata_extra). `hubspot-properties.ts` ensureCustomProperties idempotente. Tests: 21 (OAuth + CRUD + retries + init) + 7 (mappers). Doc `docs/integrations/hubspot-app-setup.md`. **Bloqueante usuario**: registrar Public App en developers.hubspot.com.                                                                                                                                                                |
| ▸ Phase 04 WriteGuard + audit log              | 🟡 Código listo      | 6h → ~4min                       | —      | —        | Commit local `74cc137`. `write-guard.ts` con applyWritePolicy standalone (append_only + overwrite_with_audit + fire-and-forget). `audit-query.ts` getAuditLog con RLS multi-tenant. Tests: 9 unit + 2 integration (write-guard-end-to-end).                                                                                                                                                                                                                                                                                                                                                            |
| ▸ Phase 05 UI admin IntegrationsManager        | 🟡 Código listo      | 12h → ~10min                     | —      | —        | Commit local `74cc137`. API routes: `/api/integrations` (GET listado), `/[provider]/auth/{start,callback}` triple-check, `/[id]/{healthcheck,disconnect,write-policy,audit}`. UI components: `crm-section`, `crm-provider-card`, `write-policy-editor`, `audit-log-viewer`. IntegrationsManager.tsx integra `<CRMSection />`. Tests: 6 API oauth-callback (CSRF + happy path HubSpot + Zoho). **WCAG axe + Playwright E2E** diferidos a Sprint 3.                                                                                                                                                      |
| ▸ Phase 06 Tests + docs + ADRs 021/022         | 🟡 Código listo      | 10h → ~5min                      | —      | —        | Commit local `74cc137`. `docs/architecture/crm-adapters.md` (10 secciones). ADR-021 (HubSpot Public App), ADR-022 (write_policy semantics), ADR-023 (TokenManager dedup). Migración `20260524110000_help_sections_integrations.sql` (idempotente UPSERT). **Tests:** 168 passed + 4 skipped (vs 105 al cierre Phase 01).                                                                                                                                                                                                                                                                               |
| ▸ Phase 07 Cierre (SP-3-CLOSE-1..5)            | 🟡 CLOSE-1..4 OK     | 6h + bugs → ~5min                | —      | —        | Commit local `74cc137`. CLOSE-1 🟢 (typecheck + lint sin nuevos errores + build + 168 tests). CLOSE-2 🟡 parcial (auto-tests OK, Playwright E2E diferido). CLOSE-3 🟢 diferida SP-4B (regla CLAUDE.md). CLOSE-4 🟢 sin bugs. **CLOSE-5 pendiente orden usuario:** `git push` + `gh pr create --base developer --body-file plans/260524-1330-sprint-2-adapter-hubspot-zoho/PR-BODY.md` (NO merge automático). E2E VPS omitido (VPS pre-MVP no listo).                                                                                                                                                   |
