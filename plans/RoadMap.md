---
title: "RoadMap dashboard-esden"
audience: equipo de desarrollo
status: LIVING_DOCUMENT
maintained_by: agente `roadmap-keeper` (proactivo) — orquestado por `esden-agents:manager`
last_updated: 20-05-2026 14:00
last_updated_by: roadmap-keeper (Sprint 1: añadidas 1-25 1-26 desde ADR audit + Sprint 2: añadido bloque 2.8 hardening deps) + renombrado a sprints numéricos 20-05-2026
project_version: v0.0.0
excluded_from: [staging, main]
---

# RoadMap — dashboard-esden

> ⚠️ **Documento vivo**. Mantenido proactivamente por el agente [`esden-agents:roadmap-keeper`](../.claude/agents/roadmap-keeper.md). NO editar directamente sin orden del lead — pide al agente que lo haga.
>
...
| **Fase 5 (post-MVP)**                    | ~4-7 semanas      |
| **Versión objetivo MVP**                 | `v0.4.0`          |

---

## Fase 1 — Sprint 1: Hotfixes de seguridad
...
| **Rama de trabajo sugerida**   | `feature/sp-1-sprint-1-hotfixes`  |
| **Inicio**                     | —                                 |
| **Fin Est.**                   | —                                 |
| **Fin Real**                   | —                                 |
...
| 1-06 | Crear usuario Postgres `app_user` con permisos limitados (no superuser) | 3h         | 🔘 Pendiente | R-023.a         | SQL script reusable luego en Fase 2. Verificar en kickoff si worker.js usa pg directo. Si sí: +1h por cadena de conexión worker. |
...
| 1-09 | Guard condicional `tenants.test_orchestrator_enabled` (deny by default)                                                                       | 2h         | 🔘 Pendiente | DA-3-003            | TEMPORAL — eliminar en Fase 4      |
...
| ID                                 | Tarea                                                                         | Estimación      | Estado      | Refs audit                            | Notas                                                                                                                             |
| ---------------------------------- | ----------------------------------------------------------------------------- | --------------- | ----------- | ------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| 1-22                               | Fix SSRF `/api/tenant/migrate` cookie `esden-tenant-url` (añadir allowlist)   | 8h              | 🔘 Pendiente | DA-3-002                              | Cookie editable por JS = vector activo. +2h por allowlist dinámica por tenant                                                     |
...
| 1-24                               | Update `axios@1.14.0` (15 CVEs: SSRF + Prototype Pollution)                   | 4h              | 🔘 Pendiente | DA-3-CVE-001                          | Pasar por `esden-agents:adr` antes                                                                                                |
| 1-25                               | Reemplazar paquete `crypto@1.0.1` DEPRECATED por built-in `node:crypto`       | 3h              | 🔘 Pendiente | ADR-2026-05-20                        | Imports en todo el src/ y worker.js. Vía esden-agents:adr                                                                         |
| 1-26                               | Update `next@16.1.6` → `next@16.2.6` (cierre 19 CVEs incl. middleware bypass) | 4h              | 🔘 Pendiente | DA-3-CVE-002                          | **MOVIDA DESDE 2-27.** Pre-requisito de 1-07, 1-08, 1-16, 1-17 (sin esto el middleware sigue bypassable). Vía esden-agents:adr   |
| **Subtotal Fase 1 — Desarrollo**   |                                                                               | **~100h 30min** |             | (con paralelismo 2 devs: ~50h reales) |                                                                                                                                   |

> Aplazados a Fase 2: cifrar Google OAuth tokens (DA-3-006, L).
> **Nota:** 2-27 (update Next.js, DA-3-CVE-002) MOVIDA a Sprint 1 como **1-26** tras hallazgo en auditoría ADR del 20-05-2026 (el middleware bypass anula efectivamente los hotfixes de auth 1-07, 1-08, 1-16).
...
| ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------ | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| SP-1-CLOSE-1                  | **Auto test** — `npm run typecheck` + `npm run lint` + `npm run build` + `npm test` (unit + integration). Reporte de coverage.                                                                           | 1h 30min                       | 🔘 Pendiente | Delegado a `esden-agents:testing`                                                                                                                           |
| SP-1-CLOSE-2                  | **Test E2C Local** — Abrir browser con Playwright, recorrer flujos implementados, validar visual + diseño + **WCAG 2.2 AA**. Generar reporte con screenshots de pasos clave + findings de accesibilidad. | 2h 30min                       | 🔘 Pendiente | Delegado a `esden-agents:testing` + `esden-agents:uxui`                                                                                                     |
...
| SP-1-CLOSE-4                  | **Corrección de Bugs y cambios detectados** — Subtareas dinámicas: una por cada bug/cambio que reporte el dev. Cada subtarea con su propio estado. Esta tarea queda 🟡 mientras haya subtareas abiertas.  | (variable)                     | 🔘 Pendiente | Delegado a `esden-agents:code` + `esden-agents:debugger`                                                                                                    |
| SP-1-CLOSE-5                  | **Cierre de Sprint** — PR `feature/sp-1-sprint-1-hotfixes` → `developer`. Tras merge: bump SemVer a `v0.1.0`, invitar al dev a tomar siguiente sprint, crear rama `feature/sp-2-capa-datos`.             | 30min                          | 🔘 Pendiente | Delegado a `esden-agents:git` (verifica estados previos) + `esden-agents:deployment` (gatekeeper changelog) + `esden-agents:productivity` (cierre tracking) |
| **Subtotal cierre Sprint 1**  |                                                                                                                                                                                                          | **5h 30min + Corrección bugs** |             |                                                                                                                                                             |
...
- [ ] `CHANGELOG.md` con entrada `## [v0.1.0]` completa (gatekeeper `esden-agents:deployment`).
- [ ] `help-docs-keeper` actualizó secciones de ayuda afectadas, todas en 🟢 Completada.

---

## Fase 2 — Sprint 2: Capa de datos (sin ORM nuevo)
...
### Tareas de desarrollo (Fase 2) — DETALLADAS
...
| 2-03 | Eliminar JWTs `service_role` residuales (los que sobrevivieron Sprint 1)               | 3h         | 🔘 Pendiente | Continuación de 1-04                                |
...
| 2-11 | Zod schemas: `integrations` / `webhooks` / `crm_field_mapping` / `crm_write_audit` | 3h         | 🔘 Pendiente | Prep para Fase 3                                 |
...
| 2-26 | Cifrar Google OAuth tokens en JSONB (no plano)                                     | 12h        | 🔘 Pendiente  | DA-3-006. Aplazado desde Sprint 1  |
| 2-27 | ~~Update next@16.1.6~~ **MOVIDA a Sprint 1 como 1-26 (tras audit ADR 20-05-2026)** | ~~6h~~ —   | ✅ Reasignada | Ver fila 1-26                      |

#### Bloque 2.7 — Testing y documentación

| ID   | Tarea                                                                            | Estimación | Estado      | Notas                                   |
| ---- | -------------------------------------------------------------------------------- | ---------- | ----------- | --------------------------------------- |
| 2-28 | Tests de integración con BD real (NO mocks) para repositorios principales        | 12h        | 🔘 Pendiente | Suite mínima — más en Fase 4            |
| 2-29 | Documentar capa de datos en `docs/architecture/data-layer.md` (refresh completo) | 4h         | 🔘 Pendiente | Delegado a `esden-agents:documentation` |

#### Bloque 2.8 — Hardening de dependencias (hallazgos ADR audit 2026-05-20)

| ID                                | Tarea                                                                                  | Estimación | Estado      | Notas                                                                            |
| --------------------------------- | -------------------------------------------------------------------------------------- | ---------- | ----------- | -------------------------------------------------------------------------------- |
| 2-30                              | Crear hook `esden-productivity-logger.cjs` para automatizar tracking de tiempos        | 6h         | 🔘 Pendiente | Ver `plans/260520-1342-sistema-logs-tiempo-sprints/phase-03-hook-integration.md` |
...
| **Subtotal Fase 2 — Desarrollo**  |                                                                                        | **~172h**  |             | (con paralelismo 2-3 devs: ~75-95h reales = 3-4 sem)                             |
...
| SP-2-CLOSE-5                  | Cierre de Sprint → PR a `developer` + bump a `v0.2.0` + crear rama Sprint 3  | 30min               | 🔘 Pendiente |                                  |
| **Subtotal cierre Sprint 2**  |                                                                              | **5h 30min + bugs** |             |                                  |

---

## Fase 3 — Sprint 3: Adapter layer + 2 CRMs (MVP)
...
> MVP de integraciones: HubSpot + Zoho. **Sheets queda fuera del MVP** (aplazado a Fase 5).
...
| SP-3-CLOSE-5                  | Cierre de Sprint → PR a `developer` + bump a `v0.3.0` + crear rama Sprint 4  | 30min               | 🔘 Pendiente |                                     |
| **Subtotal cierre Sprint 3**  |                                                                              | **5h 30min + bugs** |             |                                     |

---

## Fase 4 — Sprint 4: Hardening
...
| **Subtotal Fase 4 — Desarrollo**  |                                                                     | **(pendiente estimar)** |             |                          |
...
| SP-4-CLOSE-5                  | Cierre de Sprint → PR a `developer` + **bump a `v0.4.0`** + invitar a planificar Fase 5 (o cierre MVP)   | 30min         | 🔘 Pendiente | MVP completo                   |
| **Subtotal cierre Sprint 4**  |                                                                                                           | **8h + bugs** |             |                                |

---

## Fase 5 — Sprint 5 (post-release)
...
| ------------------ | ---------------- | ----------- | --------------------------------------- | ----------------------------------- | --------------- |
| **1** (Sprint 1)   | v0.1.0           | 🔘 Pendiente | **26** (detalladas)                     | ~100h 30min (2 sem con paralelismo) | 5h 30min + bugs |
| **2** (Sprint 2)   | v0.2.0           | 🔘 Pendiente | **33** (detalladas, con 1 movida a 1)   | ~172h (3-4 sem con paralelismo)     | 5h 30min + bugs |
| **3** (Sprint 3)   | v0.3.0           | 🔘 Pendiente | 7 (placeholder — detallar al arrancar)  | 2-3 sem                             | 5h 30min + bugs |
| **4** (Sprint 4)   | v0.4.0           | 🔘 Pendiente | 7 (placeholder)                         | 2-3 sem                             | 8h + bugs       |
| **5** (Sprint 5)   | v0.5.0+          | 🔘 Pendiente | 6 (placeholder)                         | 4-7 sem                             | ~9-12h + bugs   |
...
**Última actualización**: 20-05-2026 14:00 por `roadmap-keeper` (Sprint 1 v3 + Sprint 2 con hallazgos ADR audit). Renombrado letras→números aplicado 20-05-2026.
