...
- `crm_field_mapping` y `crm_write_audit` en 2-11 son prep para Fase 3 — diseñar pensando en la interfaz del adapter
...
8. **2-11 — Integrations + CRM (3h)**
   ...
   - `CrmFieldMappingSchema` — diseñar pensando en Fase C: `{ crm_type, crm_field, local_field, transform? }`
...
- [ ] 2-11: `src/lib/schemas/integrations.ts` (prep Fase C)
...
- **Revisión:** `esden-agents:code` (verificar naming contra VARIABLES DEFINIDAS)

## Next Steps

- Completar antes de iniciar Fase 3 (Repository pattern depende de los tipos Zod)
- Los tipos de `integrations.ts` los usará Fase 3 (adapter layer HubSpot/Zoho)
