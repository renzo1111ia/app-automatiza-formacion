# Phase 08 — Hook Automation: esden-productivity-logger.cjs
...
- Hook existente: `.claude/hooks/esden-task-tracker.cjs` — analizado, no modificar (hint-only)
- Agente productivity: `.claude/agents/productivity.md` — ya actualizado con nuevo rol

## Overview

- **Prioridad:** P2
- **Estado:** Pendiente — independiente, puede empezar en cualquier momento del Sprint 2
- **Descripción:** Crear el hook `esden-productivity-logger.cjs` que detecta automáticamente cambios de estado en `RoadMap.md` e invoca a `roadmap-keeper` y `productivity` en paralelo. En Sprint 1 el flujo es manual; este hook lo automatiza para Sprint 2 en adelante.
- **Estimación:** 7h (max) / 3h (min) — ver 2-30.1 + 2-30.2
- **Agente:** `esden-agents:code`
...
- El hook actual `esden-task-tracker.cjs` es PostToolUse hint-only — NO detecta estados, NO invoca agentes
...
- Si spike = SI: implementar hook `esden-productivity-logger.cjs` que invoca agentes en paralelo
...
- Path A: `.claude/hooks/esden-productivity-logger.cjs`
- Path B: `scripts/invoke-manager.cjs` (script manual)

**Leer para contexto:**
- `.claude/hooks/esden-task-tracker.cjs` — hook existente como referencia
...
**No modificar:**
- `.claude/hooks/esden-task-tracker.cjs` — mantener intacto
...
3. Revisar `.claude/settings.json` y `.claude/hooks/esden-task-tracker.cjs` para entender el modelo de ejecución actual
...
1. Crear `.claude/hooks/esden-productivity-logger.cjs`
...
- **Responsable:** `esden-agents:code` (2-30.1 + 2-30.2)
- **Revisión:** `esden-agents:code` (code review del hook/script antes de activar)
...
- Si Path A: hook activo desde inicio de Sprint 2 — automatiza tracking de tiempo
