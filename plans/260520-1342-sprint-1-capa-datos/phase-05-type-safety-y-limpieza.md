...
- ADR report: `plans/reports/adr-auditoria-dependencias-20260520.md` §Sprint 1 y §Findings High
  - HIGH-001: lucide-react major (2-31)
  - HIGH-002: shadcn major → aplazado a Sprint 3 (ver nota abajo)
  - HIGH-003: @types/node desalineado (2-33)
  - HIGH-004: eslint 10 — aplazado (2-34, solo investigación)
  - MED-001: @supabase/ssr upgrade (absorbido en Fase 0)
  - MED-002/005: langchain upgrade (2-35 nuevo)

## Overview

- **Prioridad:** P1 (2-22) / P2 (2-31..2-34)
- **Estado:** Pendiente — paralelizable con Fase 3 (tocan archivos distintos)
- **Descripción:** Eliminar los 426 `as any`/`as unknown` usando tipos derivados Zod. Ejecutar updates de dependencias identificados en ADR para Sprint 1. 2-22 es la tarea más larga del sprint — debe hacerse junto con el refactor de queries para aprovechar el contexto.
...
5. **2-22 — Eliminar as any (16h — tarea continua)**
   - Fase 0 (4h): archivos de Fases 01-04 ya abiertos — sustituir `as any` por `z.infer<>`
   - Fase 1 (6h): `src/app/api/**` y `src/lib/actions/**` post-refactor
   - Fase 2 (6h): `src/components/**` — usar tipos de props React + tipos derivados Zod
...
- [ ] 2-22 Fase 0: eliminar as any en archivos Fases 01-04
- [ ] 2-22 Fase 1: eliminar as any en src/app/api/ y src/lib/actions/
- [ ] 2-22 Fase 2: eliminar as any en src/components/
...
- **Responsable:** `af-agents:code`
- **ADR pre-install:** `af-agents:adr` (2-31, 2-33, 2-35)
- **Revisión:** `af-agents:uxui` (review visual post lucide-react)
