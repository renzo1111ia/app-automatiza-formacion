...
- **Prioridad:** P1
- **Estado:** Pendiente — requiere Fase 0 (cliente unificado) + Fase 1 (schemas Zod)
...
- El repository de integrations es prep para Fase 2 (el adapter HubSpot/Zoho usará `IntegrationsRepository`)
...
**Leer para contexto:**
- `plans/20260519-1200-rls-multitenant-hardening/phase-05-repository-pattern-zod.md`
- `src/lib/schemas/` (todos los schemas creados en Fase 1)
...
7. **2-18 — Repository integrations + webhooks (3h)**
   ...
   - Diseñar interfaz pensando en Fase 2: `findByCrmType('hubspot' | 'zoho')`
...
- **Revisión:** `af-agents:security` (verificar tenant isolation en cada método)

## Next Steps

- Fase 3 (refactor queries) consume los repositories de esta fase
- Fase 7 (testing) escribe tests de integración sobre los repositories de esta fase
- Fase 2 usa `IntegrationsRepository` y `LeadsRepository` directamente
