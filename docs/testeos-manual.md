...
- Para reproducir, los pasos asumen `.env.local` configurado y `npm run dev` corriendo (puerto 3000 o el que reporte el server).
...
### Tabla de tests manuales — Sprint 0

| #        | Estado | Test                                                        | Pasos / Resultado esperado                                                                                                                                                                                               | Notas |
| -------- | :----: | ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----- |
| SP0-T-01 | `[ ]`  | **Login + cookie de sesión**                                | Abrir `/login` → introducir credenciales válidas → debe redirigir a `/dashboard` y crear cookie `sb-*-auth-token`.                                                                                                       |       |
| SP0-T-02 | `[ ]`  | **Acceso a `/dashboard/settings` solo admin**               | Login como user **no admin** → entrar a `/dashboard/settings` → debe redirigir a `/dashboard` (no debe renderizar contenido).                                                                                            |       |
| SP0-T-03 | `[ ]`  | **Admin sí ve `/dashboard/settings`**                       | Login como user admin (recordar: tras 1-16, `app_metadata.is_admin = true`) → `/dashboard/settings` renderiza completo.                                                                                                  |       |
| SP0-T-04 | `[ ]`  | **Listado de tenants solo admin (1-17)**                    | Como user no admin abrir `/dashboard/admin` (o llamar internamente `getTenants()`) → debe devolver lista vacía. Como admin → ve los tenants.                                                                             |       |
| SP0-T-05 | `[ ]`  | **Crear tenant solo admin (1-17)**                          | Como user no admin intentar `createTenant` desde el panel → mensaje "Acción requiere rol admin." Como admin → tenant creado OK.                                                                                          |       |
| SP0-T-06 | `[ ]`  | **Borrar tenant solo admin (1-17)**                         | Como no admin intentar borrar → error. Como admin → borrado correcto.                                                                                                                                                    |       |
| SP0-T-07 | `[ ]`  | **API orquestación rechaza anónimos (1-07)**                | `curl http://localhost:3000/api/orchestration/workflows?tenantId=<uuid>` sin cookie → `401 Unauthorized`.                                                                                                                |       |
| SP0-T-08 | `[ ]`  | **API orquestación rechaza tenant ajeno (1-07)**            | Como user de tenant A, llamar al API con `tenantId` del tenant B → `403 Forbidden`.                                                                                                                                      |       |
| SP0-T-09 | `[ ]`  | **Cron sweep exige secret (1-08)**                          | `curl http://localhost:3000/api/orchestration/sweep` sin header → `401` (o `503` si `CRON_SECRET` falta). Con `-H "x-cron-secret: $CRON_SECRET"` → 200.                                                                  |       |
| SP0-T-10 | `[ ]`  | **Cron reminders exige secret (1-08)**                      | Idem a SP0-T-09 sobre `/api/cron/appointments/reminders`.                                                                                                                                                                |       |
| SP0-T-11 | `[ ]`  | **Orchestration disabled por defecto (1-09)**               | Como admin de un tenant SIN `tenants.config.test_orchestrator_enabled = true`, intentar publicar un workflow → `403 Orchestration disabled`. Habilitar el flag en Supabase Studio → ahora sí 200.                        |       |
| SP0-T-12 | `[ ]`  | **`/api/admin/tenants/[id]/client-sql` exige admin (1-10)** | Anónimo o user no admin → 401/403. Admin → 200 con SQL adjunto.                                                                                                                                                          |       |
| SP0-T-13 | `[ ]`  | **`/api/tenant/migrate` GET exige admin (1-11)**            | `curl http://localhost:3000/api/tenant/migrate` anónimo → `401`.                                                                                                                                                         |       |
| SP0-T-14 | `[ ]`  | **Webhook Retell sin firma → 401 (1-12)**                   | `curl -X POST http://localhost:3000/api/webhooks/retell -H "content-type: application/json" -d '{"event":"call_ended"}'` → `401` o `503`.                                                                                |       |
| SP0-T-15 | `[ ]`  | **Webhook Retell tools sin firma → 401 (1-13)**             | Idem sobre `/api/webhooks/retell/tools` con payload `{"name":"book_appointment","args":{},"call":{"metadata":{}}}`.                                                                                                      |       |
| SP0-T-16 | `[ ]`  | **WhatsApp webhook sin secret env → 503 (1-14)**            | Borrar temporalmente `WHATSAPP_APP_SECRET` del `.env.local`, reiniciar dev, `curl -X POST /api/webhooks/whatsapp -d '{}'` → `503 Service misconfigured`. Restaurar la env var.                                           |       |
| SP0-T-17 | `[ ]`  | **WhatsApp webhook sin header signature → 401 (1-14)**      | Con env configurada, `curl -X POST /api/webhooks/whatsapp -d '{}'` (sin `x-hub-signature-256`) → `401 Missing signature`.                                                                                                |       |
| SP0-T-18 | `[ ]`  | **CRM webhook sin x-tenant-id → 400 (1-15)**                | `curl -X POST /api/webhooks/crm -d '{}'` → `400 Missing x-tenant-id header`.                                                                                                                                             |       |
| SP0-T-19 | `[ ]`  | **CRM webhook con tenant pero sin firma → 401 (1-15)**      | `curl -X POST /api/webhooks/crm -H "x-tenant-id: <uuid>" -d '{}'` → `401/403/503`.                                                                                                                                       |       |
| SP0-T-20 | `[ ]`  | **CRM webhook con firma válida → 200 (1-15)**               | Configurar `tenants.config.webhook_crm_secret` para un tenant test. Calcular HMAC-SHA256 de un body de prueba con ese secret. `curl -X POST -H "x-tenant-id: <uuid>" -H "x-webhook-signature: <hex>" -d '<body>'` → 200. |       |
| SP0-T-21 | `[ ]`  | **RLS tenants — cliente solo ve su tenant (1-18)**          | Login como user A (auth_user_id = X). En Supabase Studio o vía SQL como rol `authenticated` con JWT de X: `SELECT * FROM tenants;` → debe devolver SOLO la fila del tenant del user, no las demás.                       |       |
| SP0-T-22 | `[ ]`  | **RLS tenants — admin ve todos (1-18)**                     | Repetir SP0-T-21 con un user que tenga `app_metadata.is_admin=true` → `SELECT * FROM tenants` devuelve todas las filas.                                                                                                  |       |
| SP0-T-23 | `[ ]`  | **RLS tenants — no admin no puede INSERT (1-18)**           | Como user no admin, intentar `INSERT INTO tenants ...` → policy bloquea.                                                                                                                                                 |       |
| SP0-T-24 | `[ ]`  | **RLS knowledge_base — owner-only (1-19)**                  | Como user A, `SELECT * FROM knowledge_base WHERE tenant_id = <tenant_B>` → devuelve 0 filas aunque el row exista.                                                                                                        |       |
| SP0-T-25 | `[ ]`  | **fetchCalls filtra por tenant activo (1-20)**              | Como user de tenant A en `/dashboard/historial` → solo ve leads/llamadas de tenant A. Sin `esden-tenant-id` cookie (borrarla manual) → la tabla queda vacía.                                                             |       |
| SP0-T-26 | `[ ]`  | **IDOR inbox: editar lead ajeno falla silencioso (1-21)**   | Como user de tenant A, en la URL del inbox forzar `leadId` de un lead del tenant B → la action devuelve `success: true` pero el `update` afecta 0 filas (verificar en DB que el lead B no cambió).                       |       |
| SP0-T-27 | `[ ]`  | **IDOR inbox: borrar lead ajeno no afecta (1-21)**          | Idem para `deleteLead` con leadId de tenant B → no se borra nada.                                                                                                                                                        |       |
| SP0-T-28 | `[ ]`  | **SSRF migrate bloquea host externo (1-22)**                | Manipular `tenants.supabase_url` en DB para apuntar a `http://internal-admin.local`. Llamar `POST /api/tenant/migrate` como admin → respuesta `400 Supabase URL inválida` (red privada bloqueada).                       |       |
| SP0-T-29 | `[ ]`  | **SSRF migrate permite Supabase local en dev (1-22)**       | Con `ALLOW_INTERNAL_TENANT_URLS=true` y `tenants.supabase_url` apuntando a `http://localhost:8200`, `POST /api/tenant/migrate` como admin → 200. Sin la env var → 400.                                                   |       |
| SP0-T-30 | `[ ]`  | **Widget XSS id malicioso → 400 (1-23)**                    | `curl "http://localhost:3000/api/widget/embed.js?id=hacker';alert(1);//"` → `400 Invalid widget ID format`.                                                                                                              |       |
| SP0-T-31 | `[ ]`  | **Widget UUID válido → JS limpio (1-23)**                   | `curl "http://localhost:3000/api/widget/embed.js?id=11111111-1111-1111-1111-111111111111"` → 200, el body contiene `"11111111-..."` (JSON.stringify-safe) y NO contiene `alert(`.                                        |       |
| SP0-T-32 | `[ ]`  | **axios bump no rompe descarga de media WhatsApp (1-24)**   | Provocar un webhook WhatsApp INBOUND con un mensaje tipo `image` (mock o real) → log de `[WHATSAPP PROCESSOR] Media uploaded to MinIO` aparece sin errores `AxiosHeaderValue`.                                           |       |
| SP0-T-33 | `[ ]`  | **paquete `crypto` removido — app arranca limpio (1-25)**   | Tras `npm install`, `npm run dev` arranca sin warning "deprecated package crypto". `npm ls crypto` reporta `(empty)`.                                                                                                    |       |
| SP0-T-34 | `[ ]`  | **Next.js 16.2.6 sirve sin warnings de seguridad (1-26)**   | Arrancar `npm run dev` y `npm run build` — sin warnings de CVE en consola; rutas dinámicas de admin funcionan.                                                                                                           |       |
